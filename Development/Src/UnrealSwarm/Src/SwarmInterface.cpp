...
public:
	...
	Int32 OpenConnection( FConnectionCallback CallbackFunc, void* CallbackData, ELogFlags LoggingFlags, Process^ AgentProcess, UBOOL AgentProcessOwner, Int32 LocalProcessID, AgentConfiguration^% NewConfiguration )
	{
		...
		{
			// While the application is alive and responding, wait, but inform the callback so it can handle it
			Debug::WriteLineIf( Debugger::IsAttached, "[OpenConnection] Waiting for agent to respond ..." );
			if( CallbackFunc != NULL )
			{
				FMessage UpdateMessage( MESSAGE_UPDATE );
				CallbackFunc( &UpdateMessage, CallbackData );
			}
		}
...
INT FSwarmInterfaceManagedImpl::TryOpenConnection( FConnectionCallback CallbackFunc, void* CallbackData, ELogFlags LoggingFlags )
{
	try
	{
		...
		ConnectionHandle = Connection->OpenConnection( CallbackFunc, CallbackData, LoggingFlags, AgentProcess, AgentProcessOwner, Process::GetCurrentProcess()->Id, ConnectionConfiguration );
		StopTiming();
