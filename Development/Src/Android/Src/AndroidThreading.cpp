/**
 * AndroidThreading.cpp -- Contains all Android platform specific definitions
...
/** The global synchronization object factory.	*/
...
/**
 * Constructor that zeros the handle
 */
FEventAndroid::FEventAndroid(void)
{
	bInitialized = FALSE;
...
void FEventAndroid::LockEventMutex()
{
	const int rc = pthread_mutex_lock(&Mutex);
	check(rc == 0);
}

void FEventAndroid::UnlockEventMutex()
{
 ...
 */
void FEventAndroid::Unlock(void)
{
 ...
 */
UBOOL FEventAndroid::Create(UBOOL _bIsManualReset,const TCHAR* InName)
{
	check(!bInitialized);
	UBOOL RetVal = FALSE;
	Triggered = TRIGGERED_NONE;
 ...
 */
void FEventAndroid::Trigger(void)
{
	check(bInitialized);
 ...
 */
UBOOL FEventAndroid::Wait(DWORD WaitTime)
{
	check(bInitialized);
 ...
 * Zeroes its members
 */
FSynchronizeFactoryAndroid::FSynchronizeFactoryAndroid(void)
{
}
 ...
 */
FCriticalSection* FSynchronizeFactoryAndroid::CreateCriticalSection(void)
{
 ...
 */
FEvent* FSynchronizeFactoryAndroid::CreateSynchEvent(UBOOL bIsManualReset,
	const TCHAR* InName)
{
	// Allocate the new object
	FEvent* Event = new FEventAndroid();
 ...
 */
void FSynchronizeFactoryAndroid::Destroy(FSynchronize* InSynchObj)
{
	delete InSynchObj;
}
 ...
 * Zeros any members
 */
FQueuedThreadAndroid::FQueuedThreadAndroid(void) :
	DoWorkEvent(NULL),
	ThreadCreated(FALSE),
	TimeToDie(FALSE),
 ...
 */
void *FQueuedThreadAndroid::_ThreadProc(void *pThis)
{
	check(pThis);
	((FQueuedThreadAndroid*)pThis)->Run();
	return NULL;
}
 ...
 */
void FQueuedThreadAndroid::Run(void)
{
	while (!TimeToDie)
	{
 ...
 */
UBOOL FQueuedThreadAndroid::Create(FQueuedThreadPool* InPool,DWORD ProcessorMask,
 ...
 */
UBOOL FQueuedThreadAndroid::Kill(UBOOL bShouldWait, UBOOL bShouldDeleteSelf)
{
 ...
 */
void FQueuedThreadAndroid::DoWork(FQueuedWork* InQueuedWork)
{
 ...
 */
FQueuedThreadPoolAndroid::~FQueuedThreadPoolAndroid(void)
{
 ...
 */
UBOOL FQueuedThreadPoolAndroid::Create(DWORD InNumQueuedThreads,DWORD ProcessorMask,
	DWORD StackSize,EThreadPriority ThreadPriority)
{
	...
	if (bWasSuccessful == TRUE)
	{
			...
			Count++)
		{
			...
			FQueuedThread* pThread = new FQueuedThreadAndroid();
 ...
 * Zeroes members
 */
FRunnableThreadAndroid::FRunnableThreadAndroid(void)
{
	ThreadCreated			= FALSE;
	ThreadHasTerminated		= FALSE;
	Runnable				= NULL;
 ...
 * @return True if the thread and all of its initialization was successful, false otherwise
 */
UBOOL FRunnableThreadAndroid::Create(FRunnable* InRunnable, const TCHAR* ThreadName,
 ...
 */
void FRunnableThreadAndroid::Suspend(UBOOL bShouldPause)
{
 ...
 */
UBOOL FRunnableThreadAndroid::Kill(UBOOL bShouldWait)
{
 ...
 * thread main function
 */
void *FRunnableThreadAndroid::_ThreadProc(void *pThis)
{
	check(pThis);
	// !!! FIXME: we ignore the return value.
	//return ((FRunnableThreadAndroid*)pThis)->Run();
	((FRunnableThreadAndroid*)pThis)->Run();
	return NULL;
}
 ...
 */
DWORD FRunnableThreadAndroid::Run(void)
{
 ...
 */
void FRunnableThreadAndroid::SetThreadPriority(EThreadPriority NewPriority)
{
 ...
 */
void FRunnableThreadAndroid::SetProcessorAffinity(DWORD ProcessorNum)
{
 ...
 */
void FRunnableThreadAndroid::WaitForCompletion(void)
{
...
*/
DWORD FRunnableThreadAndroid::GetThreadID(void)
{
 ...
 */
FRunnableThread* FThreadFactoryAndroid::CreateThread(FRunnable* InRunnable, const TCHAR* ThreadName,
	UBOOL bAutoDeleteSelf,UBOOL bAutoDeleteRunnable,DWORD InStackSize,
	EThreadPriority InThreadPri)
{
	check(InRunnable);
	// Create a new thread object
	FRunnableThreadAndroid* NewThread = new FRunnableThreadAndroid();
	if (NewThread)
	{
 ...
 */
void FThreadFactoryAndroid::Destroy(FRunnableThread* InThread)
{
	delete InThread;
}
