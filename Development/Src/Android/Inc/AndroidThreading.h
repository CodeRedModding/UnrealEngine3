/**
 * AndroidThreading.h -- Contains all Android platform-specific definitions
...
#ifndef _ANDROID_THREADING_H
#define _ANDROID_THREADING_H
...
////////////////////
// @todo android: If we merge the unix code in to main branch, we could pretty easily share this
// this code with Unix and Android platforms
///////////////////

/**
 * This is the Android version of a critical section.
 */
...
#if STATS
	// TODO: Due to an as-yet-undetermined issue with Android, it
	// appears that light use of the __atomic_* routines does not
	// cause problems, but heavy SMP use (like when stats are
	// active) cause the process to crash without much useful
	// information. The workaround is to use a mutex for the
	// functions that stat tracking hammers on.
	//	
	// NOTE: This will incur some performance penalty, but will
	// avoid crashing.
	static FCriticalSection appInterlockedMutex;
#endif
...
{
#if STATS
	appInterlockedMutex.Lock();
		(*Value)++;
		INT NewValue = *Value;
	appInterlockedMutex.Unlock();
	return NewValue;
#else
	return (INT)__atomic_inc((int*)Value);
#endif
}
/**
...
FORCEINLINE INT appInterlockedDecrement(volatile INT* Value)
{
	...
	return NewValue;
#else
 	return (INT)__atomic_dec((int*)Value);
#endif
}
/**
...
FORCEINLINE INT appInterlockedAdd(volatile INT* Value,INT Amount)
{
	...
	return OldValue;
#else
	// Rather do this a better way
	INT OldValue = *Value;
	for( int i = 0; i < Amount; i++ )
	{
		__atomic_inc((int*)Value);
	}
	return OldValue;
#endif
}
...
{
	return (INT)__atomic_swap((int)Exchange, (int*)Value);
}
/**
...
{
	return __atomic_cmpxchg((int)Comperand, (int)Exchange, (int*)Dest) == 0 ? Comperand : Exchange;
}
/**
...
{
	// @todo android: we got this code from Unix code, but looking at what it returns, it seems like a structure
...
/**
 * This is the Android version of an event
 */
class FEventAndroid : public FEvent
{
...
/**
 * This is the Android factory for creating various synchronization objects.
 */
class FSynchronizeFactoryAndroid : public FSynchronizeFactory
{
public:
	/**
	 * Zeroes its members
	 */
	FSynchronizeFactoryAndroid(void);
...
/**
 * This is the Android class used for all poolable threads
 */
class FQueuedThreadAndroid : public FQueuedThread
{
	/**
 ...
 * platform specific code, while this class provides all of the Android
 * specific methods. It handles the creation of the threads used in the
 * thread pool.
 */
class FQueuedThreadPoolAndroid : public FQueuedThreadPoolBase
{
public:
	/**
	 * Cleans up any threads that were allocated in the pool
	 */
	virtual ~FQueuedThreadPoolAndroid(void);
 ...
 */
class FRunnableThreadAndroid : public FRunnableThread
{
	/**
	 * The thread handle to clean up. Must be closed or this will leak resources
	 */
...
/**
 * This is the factory interface for creating threads on Android
 */
class FThreadFactoryAndroid : public FThreadFactory
{
public:
	/**
	 * Creates the thread with the specified stack size and thread priority.
...
#endif  // define _ANDROID_THREADING_H
