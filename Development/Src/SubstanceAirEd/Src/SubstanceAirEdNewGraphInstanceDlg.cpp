//! @file SubstanceAirEdNewGraphInstanceDlg.cpp
...
BEGIN_EVENT_TABLE(WxDlgNewGraphInstance, wxDialog)
	EVT_BUTTON( wxID_OK, WxDlgNewGraphInstance::OnOK )
END_EVENT_TABLE()


WxDlgNewGraphInstance::WxDlgNewGraphInstance(UBOOL ShowCreateMaterialCheckBox/* =TRUE */):
	...
	PGNCtrl(NULL)
{
    const bool bSuccess =
		wxXmlResource::Get()->LoadDialog(
			this,
			GApp->EditorFrame,
			TEXT("ID_DLG_SBS_AIR_NEW_INSTANCE"));
    check(bSuccess);
    ...
    {
        PGNCtrl = new WxPkgGrpNameSbsCtrl( PGNPanel, -1, NULL, TRUE, ShowCreateMaterialCheckBox);
...
int WxDlgNewGraphInstance::ShowModal(const FString& InName, const FString& InPackage, const FString& InGroup)
{
    InstanceName = InName;
    Package = InPackage;
    Group = InGroup;
...
void WxDlgNewGraphInstance::EvaluatePackageAndGroup()
{
	Package = PGNCtrl->PkgCombo->GetValue();
	Group = PGNCtrl->GrpEdit->GetValue();
	InstanceName = PGNCtrl->NameEdit->GetValue();

	if (PGNCtrl->MaterialCheckbox)
	{
		CreateMaterial = PGNCtrl->MaterialCheckbox->IsChecked();
	}
	else
	{
		CreateMaterial = FALSE;
