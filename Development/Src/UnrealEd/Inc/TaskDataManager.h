...
#ifndef __TaskDataManager_h__
#define __TaskDataManager_h__
...
class FTaskDataManager
	: public FTaskDatabaseListener,		// Listens for task database responses
	  public FTickableObject			// Tickable, so we can update the task database
{
...
public:
	 ...
	 */
	void SetFocusedTaskNumber( const UINT InTaskNumber )
	{
	 ...
	 */
	UINT GetFocusedTaskNumber() const
	{
	 ...
	 */
	void StartMarkingTasksComplete( const TArray< UINT >& InTaskNumbers,
	 ...
	 */
	const FTaskDatabaseEntryDetails* FindCachedTaskDetails( const UINT InTaskNumber ) const
	{
...
private:
	...
	/** The last status that we sent to the GUI */
	ETaskDataManagerStatus::Type LastGUIStatus;

	// ...

	/** True if we need to refresh the list of filters from the server */
	UBOOL bShouldRefreshFilterNames;
	...
	UINT FocusedTaskNumber;

	/** List of task numbers that the user wants marked as complete */
	TArray< UINT > TaskNumbersToMarkComplete;
	...
	UINT CurrentlyProcessingTaskNumber;
	...
	TMap< INT, FTaskDatabaseEntryDetails > CachedTaskDetailsMap;
