 ...
 */
UBOOL FTaskDatabaseThreadRunnable::Init()
{
 ...
 */
DWORD FTaskDatabaseThreadRunnable::Run()
{
	...
	while( !bAskedToStop )
	{
		...
		if( GenericRequestCopy != NULL )
		{
			...
			TDLOG( TEXT( "TDThread: Starting work on request (Type: %i)" ), ( INT )GenericRequestCopy->RequestType );
			...
			if( GenericResponse != NULL )
			{
				...
				{
					TDLOG( TEXT( "TDThread: Completed SUCCESSFUL request (Type: %i)" ), ( INT )GenericRequestCopy->RequestType );

					GenericResponse->bFailedBecauseOfDisconnection = FALSE;
				}
				else
				{
					TDLOG( TEXT( "TDThread: Completed FAILED request (Type: %i)" ), ( INT )GenericRequestCopy->RequestType );
