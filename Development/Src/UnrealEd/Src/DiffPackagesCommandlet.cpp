...
UBOOL UDiffPackagesCommandlet::Initialize( const TCHAR* Parms )
{
		...
		else
		{
			...
			if ( FilesInPath.Num() == 1 )
			{
				...
				if ( bFirstPackage )
				{
					...
					PackageFilenames[0] = FFilename(Filename).GetBaseFilename();
					NumPackages++;
				}
				else if ( bSecondPackage )
				{
					...
					PackageFilenames[1] = FFilename(Filename).GetBaseFilename();
					NumPackages++;
				}
				else if ( bAncestorPackage )
				{
					...
					PackageFilenames[2] = FFilename(Filename).GetBaseFilename();
					NumPackages++;
				}
	...
	INT LongestLen = Max<INT>(PackageFilenames[2].Len(), Max<INT>(PackageFilenames[0].Len(), PackageFilenames[1].Len()));
	for (INT i = 0; i < 3; i++)
	{
			...
			else
			{
				...
				else if (PropTextA.Len() && PropTextB.Len())
				{
					...
					if (ObjA != NULL && ObjB != NULL)
					{
						PropTextA.Empty();
						PropTextB.Empty();
						Prop->ExportText(Index, PropTextA, (BYTE*)ObjA, (BYTE*)ObjB, ObjA, PPF_SimpleObjectText);
						Prop->ExportText(Index, PropTextB, (BYTE*)ObjB, (BYTE*)ObjA, ObjB, PPF_SimpleObjectText);
					}
