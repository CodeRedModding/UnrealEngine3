...
// Hints compiler that expression is true (not supported on iOS)
#define ASSUME(expr)
...
// iOS supports high precision for thread timing functions such as appSleep().
#ifdef SUPPORTS_HIGH_PRECISION_THREAD_TIMING
#undef SUPPORTS_HIGH_PRECISION_THREAD_TIMING
#endif
#define SUPPORTS_HIGH_PRECISION_THREAD_TIMING 1
...
//extern void appIPhoneInit(int argc, char* const argv[]);
...
{
	// !!! FIXME?  Can IPhone do this faster?
    return appInvSqrt(F);
}
...
{
	// !!! FIXME: Is this ok?  iPhone has getenv() if we need it.
