...
FSocketBSD::FSocketBSD(SOCKET InSocket,ESocketType InSocketType,const FString& InSocketDescription) :
	FSocket(InSocketType, InSocketDescription),
	Socket(InSocket),
	SavedType(InSocketType),
	SavedNonBlocking(-1),
	SavedBroadcast(-1),
	SavedReuseAddr(-1),
	SavedLinger(-1),
	SavedLingerTimeout(-1),
	SavedRecvError(-1),
	SavedSendBufferSize(-1),
	SavedReceiveBufferSize(-1)
{
}
...
/** Handle some types of errors */
void FSocketBSD::HandleError()
{
	...
	// handle totally dead socket
	if (errno == EPIPE)
	{
		Close();
	}
	// handle disconnection, try to reconnect (this can happen on backgrounding, etc)
	else if (errno == ENOTCONN)
	{
		debugf(TEXT("Socket %s disconnected, will try to reopen..."), *SocketDescription);
		Close();

		if (SavedType == SOCKTYPE_Datagram)
		{
		...
		// reset any options that had been set
		if (SavedNonBlocking != -1)
		{
			SetNonBlocking(SavedNonBlocking);
		}
		if (SavedBroadcast != -1)
		{
		...
		if (SavedReuseAddr != -1)
		{
			SetReuseAddr(SavedReuseAddr);
		}
		if (SavedLinger != -1)
		{
			SetLinger(SavedLinger, SavedLingerTimeout);
		}
		if (SavedRecvError != -1)
		{
			SetRecvErr(SavedRecvError);
		}
		if (SavedSendBufferSize != -1)
		{
			INT Unused;
			SetSendBufferSize(SavedSendBufferSize, Unused);
		}
		if (SavedReceiveBufferSize != -1)
		{
			INT Unused;
			SetReceiveBufferSize(SavedReceiveBufferSize, Unused);
		}
		Bind(SavedAddr);
	}
}
...
{	
	...
	} while ( BytesSent == SOCKET_ERROR && errno == SE_EWOULDBLOCK );
...
{
	// backup setting
	SavedNonBlocking = bIsNonBlocking;
...
{
	// backup setting
	SavedBroadcast = bAllowBroadcast;
...
{
	// backup setting
	SavedLinger = bShouldLinger;
	SavedLingerTimeout = Timeout;
...
UBOOL FSocketBSD::SetReceiveBufferSize(INT Size,INT& NewSize)
{
	...
	// backup setting
	SavedReceiveBufferSize = NewSize;
...
FSocket* FSocketSubsystemBSD::CreateStreamSocket(const FString& SocketDescription)
{
	...
	int bAllow = 1;
	setsockopt(Socket, SOL_SOCKET, SO_NOSIGPIPE, &bAllow, sizeof(bAllow));
	setsockopt(Socket, SOL_SOCKET, SO_REUSEADDR, &bAllow, sizeof(bAllow));
