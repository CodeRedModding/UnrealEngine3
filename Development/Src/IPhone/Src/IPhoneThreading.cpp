/**
 * IPhoneThreading.cpp -- Contains all IPhone platform specific definitions
...
/**
 * Puts the thread into real-time mode, which drastically improves the precision for
 * threading functions such as appSleep().
 *
 * @param ThreadHandle					Handle to the thread which will be put into real-time mode.
 * @param NormalProcessingTimeMs		Normal time the thread is running per iteration. (In milliseconds.)
 *										The OS won't pre-empt the thread until this time has passed.
 *										The OS rechecks periodically based on this timer.
 * @param ConstraintProcessingTimeMs	Maximum time the thread may run per iteration before the OS starts penalizing it. (In milliseconds.)
 * @return								TRUE if successfully set, otherwise FALSE
 */
UBOOL IPhoneThreadSetRealTimeMode(pthread_t ThreadHandle, DWORD NormalProcessingTimeMs, DWORD ConstraintProcessingTimeMs)
{
	
	// if high res timing is disabled, don't allow this call
...
/**
 * Puts the thread into normal mode.
 *
 * @param ThreadHandle		Handle to the thread which will be put into normal mode.
 * @return					TRUE if successfully set, otherwise FALSE
 */
UBOOL IPhoneThreadSetNormalMode(pthread_t ThreadHandle)
{
	thread_standard_policy Policy;
	kern_return_t Result = thread_policy_set(pthread_mach_thread_np(ThreadHandle), THREAD_STANDARD_POLICY,  (thread_policy_t)&Policy, THREAD_STANDARD_POLICY_COUNT);
 ...
 */
FEventIPhone::FEventIPhone(void)
{
	bInitialized = FALSE;
...
void FEventIPhone::LockEventMutex()
{
	const int rc = pthread_mutex_lock(&Mutex);
	check(rc == 0);
}

void FEventIPhone::UnlockEventMutex()
{
 ...
 */
UBOOL FEventIPhone::Create(UBOOL _bIsManualReset,const TCHAR* InName)
{
	check(!bInitialized);
	UBOOL RetVal = FALSE;
	Triggered = TRIGGERED_NONE;
 ...
 */
void FEventIPhone::Trigger(void)
{
	check(bInitialized);
 ...
 */
UBOOL FEventIPhone::Wait(DWORD WaitTime)
{
	check(bInitialized);
 ...
 * Zeroes its members
 */
FSynchronizeFactoryIPhone::FSynchronizeFactoryIPhone(void)
{
}
 ...
 */
FCriticalSection* FSynchronizeFactoryIPhone::CreateCriticalSection(void)
{
 ...
 */
FEvent* FSynchronizeFactoryIPhone::CreateSynchEvent(UBOOL bIsManualReset,
	const TCHAR* InName)
{
	// Allocate the new object
	FEvent* Event = new FEventIPhone();
 ...
 */
void FSynchronizeFactoryIPhone::Destroy(FSynchronize* InSynchObj)
{
	delete InSynchObj;
}
 ...
 * Zeros any members
 */
FQueuedThreadIPhone::FQueuedThreadIPhone(void) :
	DoWorkEvent(NULL),
	ThreadCreated(FALSE),
	TimeToDie(FALSE),
 ...
 */
void *FQueuedThreadIPhone::_ThreadProc(void *pThis)
{
	check(pThis);
	((FQueuedThreadIPhone*)pThis)->Run();
	return NULL;
}
 ...
 */
void FQueuedThreadIPhone::Run(void)
{
	while (!TimeToDie)
	{
 ...
 */
UBOOL FQueuedThreadIPhone::Create(FQueuedThreadPool* InPool,DWORD ProcessorMask,
 ...
 */
UBOOL FQueuedThreadIPhone::Kill(UBOOL bShouldWait, UBOOL bShouldDeleteSelf)
{
 ...
 */
void FQueuedThreadIPhone::DoWork(FQueuedWork* InQueuedWork)
{
 ...
 */
FQueuedThreadPoolIPhone::~FQueuedThreadPoolIPhone(void)
{
 ...
 */
UBOOL FQueuedThreadPoolIPhone::Create(DWORD InNumQueuedThreads,DWORD ProcessorMask,
	DWORD StackSize,EThreadPriority ThreadPriority)
{
	...
	if (bWasSuccessful == TRUE)
	{
			...
			Count++)
		{
			...
			FQueuedThread* pThread = new FQueuedThreadIPhone();
 ...
 * Zeroes members
 */
FRunnableThreadIPhone::FRunnableThreadIPhone(void)
{
	ThreadCreated			= FALSE;
	ThreadHasTerminated		= FALSE;
	Runnable				= NULL;
 ...
 * @return True if the thread and all of its initialization was successful, false otherwise
 */
UBOOL FRunnableThreadIPhone::Create(FRunnable* InRunnable, const TCHAR* ThreadName,
 ...
 */
void FRunnableThreadIPhone::Suspend(UBOOL bShouldPause)
{
 ...
 */
UBOOL FRunnableThreadIPhone::Kill(UBOOL bShouldWait)
{
 ...
 * thread main function
 */
void *FRunnableThreadIPhone::_ThreadProc(void *pThis)
{
	check(pThis);
	// !!! FIXME: we ignore the return value.
	//return ((FRunnableThreadIPhone*)pThis)->Run();
	((FRunnableThreadIPhone*)pThis)->Run();
	return NULL;
}
 ...
 */
DWORD FRunnableThreadIPhone::Run(void)
{
 ...
 */
void FRunnableThreadIPhone::SetThreadPriority(EThreadPriority NewPriority)
{
 ...
 */
void FRunnableThreadIPhone::SetProcessorAffinity(DWORD ProcessorNum)
{
 ...
 */
void FRunnableThreadIPhone::WaitForCompletion(void)
{
...
*/
DWORD FRunnableThreadIPhone::GetThreadID(void)
{
 ...
 */
FRunnableThread* FThreadFactoryIPhone::CreateThread(FRunnable* InRunnable, const TCHAR* ThreadName,
	UBOOL bAutoDeleteSelf,UBOOL bAutoDeleteRunnable,DWORD InStackSize,
	EThreadPriority InThreadPri)
{
	check(InRunnable);
	// Create a new thread object
	FRunnableThreadIPhone* NewThread = new FRunnableThreadIPhone();
	if (NewThread)
	{
 ...
 */
void FThreadFactoryIPhone::Destroy(FRunnableThread* InThread)
{
	delete InThread;
}
