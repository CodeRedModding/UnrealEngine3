...
static SBYTE* key = ( SBYTE* )AES_KEY;
...
#define GETU32(plaintext) (((DWORD)(plaintext)[0] << 24) ^ \
 ...
 */
static INT rijndaelSetupEncrypt( DWORD* rk, const BYTE* key, INT keybits )
{
	INT i = 0;
	DWORD temp;
 ...
 * @return the number of rounds for the given cipher key size.
 */
static INT rijndaelSetupDecrypt( DWORD* rk, const BYTE* key, INT keybits )
{
	INT nrounds, i, j;
	DWORD temp;
...
static void rijndaelEncrypt( const DWORD *rk, INT nrounds, const BYTE plaintext[16], BYTE ciphertext[16] )
{
...
static void rijndaelDecrypt( const DWORD *rk, INT nrounds, const BYTE ciphertext[16], BYTE plaintext[16] )
{
	DWORD s0, s1, s2, s3, t0, t1, t2, t3;
...
void appEncryptData( BYTE *Contents, DWORD NumBytes )
{
#ifdef AES_KEY
	DWORD rk[RKLENGTH(AES_KEYBITS)] = { 0 };
	INT nrounds;

	checkf( ( NumBytes & ( AES_BLOCK_SIZE - 1 ) ) == 0, TEXT( "NumBytes needs to tbe a multiple of 16 bytes" ) );
	checkf( appStrlen( AES_KEY ) >= KEYLENGTH( AES_KEYBITS ), TEXT( "AES_KEY needs to be at least %d characters" ), KEYLENGTH( AES_KEYBITS ) );
	checkf( appStrcmpANSI(AES_KEY, INVALID_AES_KEY) != 0, TEXT("You must specify a unique AES key. See AES.h!"));

#if TEST_ENCRYPTION
	TArray<BYTE> OriginalBlob;
	OriginalBlob.Add( NumBytes );
	appMemcpy( OriginalBlob.GetData(), Contents, NumBytes );
#endif

	// Set up the rk buffer
	nrounds = rijndaelSetupEncrypt( rk, ( const BYTE* )key, AES_KEYBITS );

	// Encrypt the data a block at a time
	for( DWORD Offset = 0; Offset < NumBytes; Offset += AES_BLOCK_SIZE )
	{
		rijndaelEncrypt( rk, nrounds, Contents + Offset, Contents + Offset );
	}

#if TEST_ENCRYPTION
	TArray<BYTE> DecryptedBlob;
	DecryptedBlob.Add( NumBytes );
	appMemcpy( DecryptedBlob.GetData(), Contents, NumBytes );

	appDecryptData( DecryptedBlob.GetData(), DecryptedBlob.Num() );
...
void appDecryptData( BYTE *Contents, DWORD NumBytes )
{
#ifdef AES_KEY
	DWORD rk[RKLENGTH(AES_KEYBITS)] = { 0 };
	INT nrounds;
	...
	// Set up the rk buffer
	nrounds = rijndaelSetupDecrypt( rk, ( const BYTE* )key, AES_KEYBITS );
