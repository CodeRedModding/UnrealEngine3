...
const FColor& FColorList::GetFColorByIndex( INT ColorIndex ) const
{
...
UBOOL FColorList::IsValidColorName( const TCHAR* ColorName ) const
{
	const FColor* Color = ColorsMap.FindRef( ColorName );
...
INT FColorList::GetColorIndex( const TCHAR* ColorName ) const
{
	const FColor& Color = GetFColorByName( ColorName );
	INT ColorIndex = 0;
	ColorsLookup.FindItem( &Color, ColorIndex );
	return ColorIndex;
}

const FString& FColorList::GetColorNameByIndex( INT ColorIndex ) const
{
...
UBOOL GShowColorList = FALSE;

UBOOL FColorList::Exec( const TCHAR* Cmd, FOutputDevice& Ar )
{
	if( ParseCommand( &Cmd,TEXT("ToggleShowColorList") ) )
	{
...
void FColorList::InitializeColor( const TCHAR* ColorName, const FColor* ColorPtr, INT& CurrentIndex )
{
	ColorsMap.Set( ColorName, ColorPtr );
	ColorsLookup.AddItem( ColorPtr );
...
void FColorList::LogColors()
{
	for( TColorsMap::TIterator It(ColorsMap); It; ++It )
	{
		...
		INT ColorIndex = 0;
		ColorsLookup.FindItem( ColorPtr, ColorIndex );

		debugf( TEXT( "%3i - %32s -> %s" ), ColorIndex, *ColorName, *ColorPtr->ToString() );
	}
}
