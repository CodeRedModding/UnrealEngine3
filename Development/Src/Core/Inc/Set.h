...
public:
	...
	void CountBytes(FArchive& Ar)
	{
		...
		Ar.CountBytes(HashSize * sizeof(INT),HashSize * sizeof(FSetElementId));
	}
	...
	/** Assignment operator. */
	void Add(const TArray<ElementType>& InElements)
	{
		for(INT i=0; i<InElements.Num(); i++)
		{
			Add(InElements(i));
		}
	}
     ...
     */
	void Add( const TSet<ElementType,KeyFuncs,Allocator>& OtherSet )
	{
	...
	void Sort()
	{
		...
		Elements.template Sort<ElementCompareClass<CompareClass> >();
	...
	TSet<ElementType,KeyFuncs,Allocator> Intersect(
	const TSet<ElementType,KeyFuncs,Allocator>& OtherSet
	)
	{
		TSet<ElementType,KeyFuncs,Allocator> Result;
	...
	TSet<ElementType,KeyFuncs,Allocator> Union(
	const TSet<ElementType,KeyFuncs,Allocator>& OtherSet
	)
	{
		TSet<ElementType,KeyFuncs,Allocator> Result;
	...
	TSet<ElementType,KeyFuncs,Allocator> Difference(
	const TSet<ElementType,KeyFuncs,Allocator>& OtherSet
	)
	{
		TSet<ElementType,KeyFuncs,Allocator> Result;
...
private:
	...
	public:
		...
		/** Serializer. */
		friend FArchive& operator<<(FArchive& Ar,FElement& Element)
		{
