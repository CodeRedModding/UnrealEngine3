...
#ifndef Inc_ColorList_H
#define Inc_ColorList_H
...
public:
	...
	UBOOL IsValidColorName( const TCHAR* ColorName ) const;

	/** Returns index of color. */
	INT GetColorIndex( const TCHAR* ColorName ) const;

	/** Returns a color based on index. If index is invalid, returns White. */
	const FColor& GetFColorByIndex( INT ColorIndex ) const;

	/** Resturn color's name based on index. If index is invalid, returns BadIndex. */
	const FString& GetColorNameByIndex( INT ColorIndex ) const;

	/** Returns the number of colors. */
	INT GetColorsNum() const
	{
...
protected:
	void InitializeColor( const TCHAR* ColorName, const FColor* ColorPtr, INT& CurrentIndex );
