...
/**
 * Sort elements. The sort is unstable, meaning that the ordering of equal items is not necessarily preserved.
 * exactly as above, except uses appMemswap instead of Exchange to swap elements
 *
 * @param	First	pointer to the first element to sort
 * @param	Num		the number of items to sort
 */
template<class T, class CompareClass> void SortMemswap( T* First, INT Num )
{
	struct FStack
	{
		T* Min;
	...
	Loop:
		...
		if( Count <= 8 )
		{
			...
			while( Current.Max > Current.Min )
			{
				...
				appMemswap( Max, Current.Max--, sizeof(T) );
			}
		}
		else
		{
			// Grab middle element so sort doesn't exhibit worst-cast behavior with presorted lists.
			appMemswap( &Current.Min[Count/2], &Current.Min[0], sizeof(T) );
			...
			for( ; ; )
			{
				...
				appMemswap( Inner.Min, Inner.Max, sizeof(T) );
			}
			appMemswap( Current.Min, Inner.Max, sizeof(T) );
