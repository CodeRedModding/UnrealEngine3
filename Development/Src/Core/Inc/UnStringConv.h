...
public:
	...
	FORCEINLINE ANSICHAR* Convert(const TCHAR* Source,ANSICHAR* Dest,DWORD Size)
	{
		...
		appMacWideCharToMultiByte(CodePage,Source,LengthW-1,Dest,LengthA);
#else
...
public:
...
#elif PLATFORM_MACOSX
		appMacMultiByteToWideChar(CP_ACP,Source,Dest,Length-1);
#else
	...
	/* return the string length without the null terminator */
	FORCEINLINE UINT Length(TCHAR* Dest)
	{
#if _MSC_VER
