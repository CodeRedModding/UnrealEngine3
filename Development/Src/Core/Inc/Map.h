...
#if PLATFORM_MACOSX || PLATFORM_IPHONE
...
public:
	 ...
	 */
	ValueType& FindOrAdd(KeyInitType Key)
	{
		FPair* Pair = Pairs.Find(Key);
		if( Pair )
		{
	...
	{
		const FPair* Pair = Pairs.Find(Key);
		return Pair ? Pair->Value : ValueType();
	}
...
public:
	...
	void KeySort()
	{
		Super::Pairs.template Sort<KeyComparisonClass<CompareClass> >();
	}
	...
	void ValueSort()
	{
		Super::Pairs.template Sort<ValueComparisonClass<CompareClass> >();
	}
...
public:
	 ...
	 * @param bMaintainOrder - TRUE if the Values array should be in the same order as the map's pairs.
	 */
	template<typename Allocator> void MultiFindPointer(KeyInitType Key,TArray<const ValueType*, Allocator>& OutValues,UBOOL bMaintainOrder = FALSE) const
	{
		for(typename Super::PairSetType::TConstKeyIterator It(Super::Pairs,Key);It;++It)
		{
...
public:
	...
	INT AddItem(ElementInitType Element,UBOOL bAllowDuplicates=FALSE )
	{
		...
		if(!bAllowDuplicates)
		{
			CurrentIndex = this->Find(Element);
		}
...
protected:
	...
	void SynchronizeIndexValues(INT StartIndex = 0)
	{
		...
		{
			INT* OldElementIndex = this->Find(UniqueElements(ElementIndex));
