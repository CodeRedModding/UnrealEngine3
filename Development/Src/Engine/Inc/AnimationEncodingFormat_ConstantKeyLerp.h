/*=============================================================================
	AnimationEncodingFormat_ConstantKeyLerp.h: Constant key compression.
 ...
 */
class AEFConstantKeyLerpShared : public AnimationEncodingFormatLegacyBase
{
public:
	/**
	...
	UBOOL bLooping)
{
	...
	CONSOLE_PREFETCH(&(DesiredPairs(0)));
	const INT PrefetchCount = Min(PairCount, 1);
	for (INT PairIndex=0; PairIndex<PairCount; ++PairIndex)
	{
		const BoneTrackPair& Pair = DesiredPairs(PairIndex);
		CONSOLE_PREFETCH(Atoms.GetTypedData() + Pair.AtomIndex);
	}

	for (INT PairIndex=0; PairIndex<PairCount; ++PairIndex)
	{
		INT PrefetchIndex = PairIndex + PrefetchCount;
		if (PrefetchIndex < PairCount)
		{
			CONSOLE_PREFETCH(Atoms.GetTypedData() + DesiredPairs(PrefetchIndex).AtomIndex);
		}
...
#endif // __ANIMATIONENCODINGFORMAT_CONSTANTKEYLERP_H__
