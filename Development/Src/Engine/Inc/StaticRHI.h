...
#ifndef __STATICRHI_H__
#define __STATICRHI_H__ 1
...
public:
	...
	// some compilers will want a virtual destructor in this class since it's an abstract base class
	virtual ~STATIC_RHI_CLASS_NAME() {}
	...
	#define DEFINE_STATICRHI_REFCOUNTING_FORTYPE(Type,ParentType) \
		...
		virtual DWORD GetRefCount(MAKE_STATIC_RHI_TTYPE( Resource )<RRT_##Type>* Reference) = 0;
...
public:
	...
	/** Equality operator. */
	UBOOL operator==(const MAKE_STATIC_RHI_TTYPE( ResourceReference )& Other) const
	{
	...
	friend UBOOL IsValidRef(const MAKE_STATIC_RHI_TTYPE( ResourceReference )& Ref)
	{
