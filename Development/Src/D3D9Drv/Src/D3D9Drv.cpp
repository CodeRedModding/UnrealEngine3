/*=============================================================================
	D3D9Drv.cpp: Unreal D3D RHI library implementation.
	Copyright 1998-2013 Epic Games, Inc. All Rights Reserved.
=============================================================================*/

#include "D3D9DrvPrivate.h"

FLOAT FD3D9EventNode::GetTiming()
{
	FLOAT Result = 0;
	...
	if (Timing.IsSupported())
	{
		...
		const QWORD GPUTiming = Timing.GetTiming();
...
void FD3D9DynamicRHI::PushEvent(const TCHAR* Name)
{
	if (bTrackingEvents)
	{
		...
		if (CurrentEventNode)
		{
			...
			CurrentEventNode->Children.AddItem(new FD3D9EventNode(Name, CurrentEventNode, this));
			CurrentEventNode = CurrentEventNode->Children.Last();
		}
		else
		{
			// Add a new root node to the tree
			CurrentEventNodeFrame->EventTree.AddItem(new FD3D9EventNode(Name, NULL, this));
...
void GatherStatsEventNode(FD3D9EventNode* Node, INT Depth, TMap<FString, FD3D9EventNodeStats>& EventHistogram)
{
	if (Node->NumDraws > 0 || Node->Children.Num() > 0)
	{
		Node->TimingResult = Node->GetTiming() * 1000.0f;

		FD3D9EventNodeStats* FoundHistogramBucket = EventHistogram.Find(Node->Name);
		...
		{
			FD3D9EventNodeStats NewNodeStats;
...
void DumpStatsEventNode(FD3D9EventNode* Node, FLOAT RootResult, INT Depth, INT& NumNodes, INT& NumDraws)
{
	NumNodes++;
	if (Node->NumDraws > 0 || Node->Children.Num() > 0)
	{
		...
		for (INT ChildIndex = 0; ChildIndex < Node->Children.Num(); ChildIndex++)
		{
			FD3D9EventNode* ChildNode = Node->Children(ChildIndex);
...
public:
	...
	static INT Compare(const FD3D9EventNodeStats& A, const FD3D9EventNodeStats& B)
	{
...
void FD3D9EventNodeFrame::StartFrame()
{
	EventTree.Reset();
...
void FD3D9EventNodeFrame::EndFrame()
{
...
void FD3D9EventNodeFrame::DumpEventTree()
{
	if (EventTree.Num() > 0)
	{
		FLOAT RootResult = 0;
		...
		{
			const QWORD GPUTiming = RootEventTiming.GetTiming();
		...
		TMap<FString, FD3D9EventNodeStats> EventHistogram;
		...
		INT NumNotShown = 0;
		for (TMap<FString, FD3D9EventNodeStats>::TIterator It(EventHistogram); It; ++It)
		{
			const FD3D9EventNodeStats& NodeStats = It.Value();
