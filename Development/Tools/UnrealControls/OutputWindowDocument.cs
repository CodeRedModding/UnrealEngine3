...
namespace UnrealControls
{
	...
	public class OutputWindowDocument
	{
		...
		public void AppendText(Color? TxtColor, string Txt)
		{
			...
			Color? LastColor = TxtColor;
			char LastChar = (char)0;

            //Bug handler tag definition
            String BugDataTag = "BUGDATA"; 
            ...
            foreach (String Line in Lines)
            {
                ...
                else if (ThisLine.Contains(BugDataTag))
                {
                    //Spawn the bug handler
                    ThisLine += BugDataHandler.HandleBug(ThisLine.Remove(0, ThisLine.LastIndexOf(BugDataTag) + BugDataTag.Length));
                    if (ThisLine.Contains("ERROR"))
                    {
                        CurrColor = Color.Red;
                    }
