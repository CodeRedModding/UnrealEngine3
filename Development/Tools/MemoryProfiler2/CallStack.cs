...
namespace MemoryProfiler2
{
    ...
    public class FCallStack
    {
        ...
        public List<FAllocationLifecycle> IncompleteLifecycles;
		...
		private FCallStack()
		{
			...
			IncompleteLifecycles = new List<FAllocationLifecycle>();
		}
		...
		public void ProcessMalloc( FStreamToken StreamToken, ref FAllocationLifecycle NewLifecycle )
		{
			...
			IncompleteLifecycles.Add( NewLifecycle );
			NewLifecycle = null;
		...
		public FAllocationLifecycle ProcessFree( FStreamToken StreamToken )
		{
			...
			bool bFound = false;
			for( int LifecycleIndex = 0; LifecycleIndex < IncompleteLifecycles.Count; LifecycleIndex++ )
			{
				FAllocationLifecycle Lifecycle = IncompleteLifecycles[ LifecycleIndex ];
				if( Lifecycle.LatestPointer == StreamToken.Pointer )
				{
					...
					IncompleteLifecycles.RemoveAt( LifecycleIndex );
		...
		public FAllocationLifecycle ProcessRealloc( FStreamToken StreamToken, ref FAllocationLifecycle NewLifecycle, FCallStack PreviousCallStack, FAllocationLifecycle PreviousLifecycle )
		{
			...
			{
				FAllocationLifecycle Lifecycle = IncompleteLifecycles[ LifecycleIndex ];
				if( Lifecycle.LatestPointer == StreamToken.OldPointer )
				{
			...
			if( !bFound )
			{
				...
				IncompleteLifecycles.Add( Result );
