	...
	public partial class Agent : MarshalByRefObject, IAgentInternalInterface, IAgentInterface
	{
		...
		public bool InitCache()
		{
			try
			{
				...
				{
					Log( EVerbosityLevel.Informative, Color.Red, " ......... Failed to get the cache location!" );
					return false;
				}

				Log( EVerbosityLevel.Informative, Color.DarkGreen, " ......... using cache folder '" + CacheFolder + "'" );

				// The agent staging area needs to be wiped clean on start up
				Log( EVerbosityLevel.Informative, Color.DarkGreen, " ......... recreating SwarmAgent cache staging area" );
				...
				Log( EVerbosityLevel.Informative, Color.DarkGreen, " ......... generating hashes for shared cache entries" );
				...
				{
					Log( EVerbosityLevel.Verbose, Color.DarkGreen, " ............ creating hash for " + NextFile.Name );
				...
				foreach( FileInfo NextFile in CacheFolderDirectory.GetFiles() )
				{
					Log( EVerbosityLevel.Informative, Color.DarkGreen, " ............ verifying hash for " + NextFile.Name );
					...
					if( ChannelHashValues.TryGetValue( NextFile.Name, out ExistingHash ) )
					{
						for( Int32 Index = 0; Index < ExistingHash.Length; Index++ )
						{
							...
							{
								Log( EVerbosityLevel.Informative, Color.DarkGreen, " ............     hash mismatch!" );
								break;
							}
						}
					}
			...
			{
				Log( EVerbosityLevel.Informative, Color.Red, " ......... Failed to initialize cache: " + Ex.Message );
				return false;
			}
			return true;
		}
            ...
            if( Connections.ContainsKey( ConnectionHandle ) )
            {
                ...
                if( File.Exists( FullPath ) )
                {
                    try
                    {
						...
						{
							Log( EVerbosityLevel.Verbose, Color.DarkGreen, " ......... removed hash for " + ChannelName );
						}
                        ...
                        if( ErrorCode >= 0 )
                        {
							...
							{
								Log( EVerbosityLevel.Verbose, Color.DarkGreen, " ......... (1) created hash for " + ChannelName );
							}
							else
							{
								Log( EVerbosityLevel.Verbose, Color.DarkGreen, " ......... (1) discovered duplicate hash for " + ChannelName );
							}
                    ...
                    {
                        Log( EVerbosityLevel.Informative, Color.Red, "[AddChannel] Failed to add \"" + FullPath + "\"" );
                        Log( EVerbosityLevel.Verbose, Color.Red, "[AddChannel] Exception: " + Ex.ToString() );
			...
			if( Connections.ContainsKey( ConnectionHandle ) )
			{
                ...
                if( File.Exists( FullChannelName ) )
                {
					...
					if( ChannelHashValues.Add( ChannelName, ComputeHash( File.ReadAllBytes( FullChannelName ) ) ) )
					{
						...
						Log( EVerbosityLevel.Verbose, Color.DarkGreen, " ......... (2) created hash for " + ChannelName );
					}
					else
					{
		...
		private Int32 OpenChannel_1_0( Int32 ConnectionHandle, Hashtable InParameters, ref Hashtable OutParameters )
		{
            ...
            if( Connections.TryGetValue( ConnectionHandle, out ConnectionThatOwnsChannel ) )
            {
				...
				Log( EVerbosityLevel.ExtraVerbose, Color.DarkGreen, "[OpenChannel] Opening Channel: " + ChannelName );
							...
							else if( ( ChannelFlags & EChannelFlags.ACCESS_READ ) != 0 )
							{
										 ...
										 ( ConnectionThatOwnsChannel.Parent is RemoteConnection ) )
								{
									...
									if( !ChannelHashValues.TryGetValue( ChannelName, out LocalHashValue ) )
									{
										...
										{
											Log( EVerbosityLevel.Verbose, Color.DarkGreen, " ......... (3) created hash for " + ChannelName );
										}
									}
									...
									{
										Log( EVerbosityLevel.Informative, Color.Orange, "[OpenChannel] Error: missing hash table entry for existing channel, pulling " + ChannelName );
									}
									// If we fail the hash comparison test, we need to pull the file
									if( !ChannelHashIsValid )
									{
										Log( EVerbosityLevel.Informative, Color.Orange, "[OpenChannel] Remote hash comparison failed, re-pulling channel " + ChannelName );
										CacheIsHit = false;
								...
								if( File.Exists( FullyCachedChannelName ) )
								{
									...
									{
										// Add the entry into the channel hash table
										Log( EVerbosityLevel.Verbose, Color.DarkGreen, " ......... (4) created hash for " + ChannelName );
									}
									else
									{
										// TODO: Add an option for fully validated cache usage and, when
				...
				{
					Log( EVerbosityLevel.Informative, Color.Red, "OpenChannel failed: " + Ex.ToString() );
					ErrorCode = Constants.ERROR_EXCEPTION;
				}
						...
						if( ( ChannelToClose.Flags & EChannelFlags.ACCESS_WRITE ) != 0 )
						{
							if( ( ChannelToClose is JobChannel ) == false )
							{
                                ...
                                if( DstChannel.Exists )
								{
									...
									Log( EVerbosityLevel.Verbose, Color.DarkGreen, " ......... removing hash for " + ChannelToClose.Name );
								...
								{
									Log( EVerbosityLevel.Verbose, Color.DarkGreen, " ......... (5) created hash for " + ChannelToClose.Name );
								}
		...
		public bool PushChannel( RemoteConnection Remote, string ChannelName, AgentGuid JobGuid )
		{
			...
			if( LocalChannelHandle >= 0 )
			{
				...
				{
                    Log( EVerbosityLevel.Informative, Color.Red, "[PushChannel] Pushing the channel has failed!" );
                    Log( EVerbosityLevel.Verbose, Color.Red, "[PushChannel] Exception message: " + Ex.ToString() );
				}
                ...
                Log( EVerbosityLevel.Verbose, Color.DarkGreen, "[Channel] Successful channel push of " + ChannelName );
			}
		...
		public bool SendChannel( Int32 ConnectionHandle, string ChannelName, byte[] ChannelData, AgentGuid JobGuid )
		{
				...
				ConnectionRequestingChannel is RemoteConnection )
			{
				...
				if( LocalChannelHandle >= 0 )
				{
					...
					{
						Log( EVerbosityLevel.Informative, Color.Orange, "[SendChannel] Channel \"" + ChannelName + "\" not transferred because of name collision. Channel with that name already exists" );
						Log( EVerbosityLevel.Verbose, Color.Orange, "[SendChannel] Exception: " + Ex.ToString() );
					}
		...
		public bool PullChannel( RemoteConnection Remote, string ChannelName, AgentGuid JobGuid )
		{
				   ...
				   ( bChannelTransferred == false ) )
			{
				...
				{
					Log( EVerbosityLevel.Informative, Color.Red, "[PullChannel] Pulling the channel has failed!" );
					Log( EVerbosityLevel.Verbose, Color.Red, "[PullChannel] Exception message: " + Ex.ToString() );
				}
			}
				...
				lock( ChannelHashValues )
				{
					if( !ChannelHashValues.TryGetValue( ChannelName, out LocalChannelHashValue ) )
					{
						...
						if( File.Exists( FullyCachedChannelName ) )
						{
							...
							{
								Log( EVerbosityLevel.Verbose, Color.DarkGreen, " ......... (6) created hash for " + ChannelName );
							}
						}
					}
				}
				...
				if( LocalChannelHashValue.Length > 0 )
				{
					...
					if( RemoteChannelHashValue.Length == LocalChannelHashValue.Length )
					{
						...
						for( Int32 Index = 0; Index < LocalChannelHashValue.Length; Index++ )
						{
							...
							{
								Log( EVerbosityLevel.Informative, Color.Orange, String.Format( "[ValidateChannel] Hash value mismatch for {0}", ChannelName ) );
								return false;
							}
						}
					...
					{
						Log( EVerbosityLevel.Informative, Color.Orange, String.Format( "[ValidateChannel] Remote channel hash is not the same length as the local one: {0} != {1}", RemoteChannelHashValue.Length, LocalChannelHashValue.Length ) );
					}
				}
				else
				{
					Log( EVerbosityLevel.Informative, Color.Orange, String.Format( "[ValidateChannel] Remote channel name not found in local hash table: {0}", ChannelName ) );
				}
			}
			else
			{
				Log( EVerbosityLevel.Informative, Color.Red, String.Format( "[ValidateChannel] Connection not valid: {0:X8}", ConnectionHandle ) );
			}
			return false;
		}
		...
		public string GetCacheLocation()
		{
            ...
            if( CacheFolder.Length == 0 )
            {
                ...
                Log( EVerbosityLevel.Critical, Color.Red, "ERROR: Could not create cache folder '" + CacheFolder + "'\nPlease update the SwarmAgent.exe.config file in your binaries folder." );
            }
		...
		public void RequestCacheRelocation()
		{
			...
			{
				Log( EVerbosityLevel.Informative, Color.DarkGreen, "[RequestCacheRelocation] Request delayed because of active, open connections" );
				Log( EVerbosityLevel.Informative, Color.DarkGreen, "[RequestCacheRelocation] Relocation will happen after all connections are closed" );
			}
		...
		public void RequestCacheClear()
		{
			...
			{
				Log( EVerbosityLevel.Informative, Color.DarkGreen, "[RequestCacheClear] Request ignored because of active, open connections" );
			}
		}
		...
		public void RequestCacheValidate()
		{
			...
			{
				Log( EVerbosityLevel.Informative, Color.DarkGreen, "[RequestCacheValidate] Request ignored because of active, open connections" );
			}
		}
		...
		public void CacheAgingThreadProc()
		{
			try
			{
				...
				Log( EVerbosityLevel.Verbose, Color.DarkGreen, "[MaintainCache] Cleaning up main cache directory" );
					...
					if( RemainingSizeToKeep < 0 )
					{
						...
						try
						{
							if( FilesDeletedCount == 0 )
							{
								...
								{
									Log( EVerbosityLevel.Informative, Color.Orange, "[MaintainCache] Deleting a cache entry that is newer than the oldest job kept" );
									Log( EVerbosityLevel.Informative, Color.Orange, "[MaintainCache] Consider increasing the size of the cache (in Settings)" );
								}
							}
						...
						{
							Log( EVerbosityLevel.Verbose, Color.Red, "[MaintainCache] Error cleaning up " + NextCacheFile.FullName );
							Log( EVerbosityLevel.Verbose, Color.Red, "[MaintainCache] Exception: " + Ex.Message );
						}
					}
					else
					{
			...
			catch( Exception Ex )
			{
				...
				Log( EVerbosityLevel.ExtraVerbose, Color.Red, "[MaintainCache] Exception: " + Ex.Message );
			}
		}
		...
		public void MaintainCache()
		{
			...
			lock( Connections )
			{
				...
				if( Connections.Count == 0 )
				{
					...
					{
						Log( EVerbosityLevel.Informative, Color.DarkGreen, "[CleanCache] Clearing out the cache by user request" );
						...
						foreach( string PersistentCacheEntry in PersistentCacheContents )
						{
							...
							{
								Log( EVerbosityLevel.Informative, Color.Orange, "[CleanCache] Failed to delete the file " + PersistentCacheEntry + ", it may be in use..." );
								Log( EVerbosityLevel.Informative, Color.Orange, "[CleanCache] Exception was: " + Ex.Message.TrimEnd( '\n' ) );
								CacheCleared = false;
							}
						}
						if( CacheCleared )
						{
							Log( EVerbosityLevel.Informative, Color.DarkGreen, "[CleanCache] Cache successfully cleared" );
						}
						else
						{
							Log( EVerbosityLevel.Informative, Color.DarkGreen, "[CleanCache] Cache not cleared, some files were not deleted, but the Agent will pretend they're gone" );
						}
					...
					else if( CacheValidateRequested )
					{
						...
						Log( EVerbosityLevel.Informative, Color.DarkGreen, "[MaintainCache] Validating all cache entries and hashes" );
						DirectoryInfo CacheFolderDirectory = new DirectoryInfo( AgentApplication.Options.CacheFolder );
						...
						foreach( FileInfo NextFile in CacheFolderDirectory.GetFiles() )
						{
							...
							if( LocalConnectionRequestWaiting.WaitOne( 0 ) )
							{
								...
								Log( EVerbosityLevel.Informative, Color.DarkGreen, "[MaintainCache] Canceling validation due to incoming local connection" );
								AllFilesUpdated = false;
								break;
							}

							Log( EVerbosityLevel.Informative, Color.DarkGreen, " ............ validating hash for " + NextFile.Name );
							...
							if( ChannelHashValues.TryGetValue( NextFile.Name, out ExistingHash ) )
							{
								...
								if( !HashMatches )
								{
									Log( EVerbosityLevel.Informative, Color.Orange, " ............     hash mismatch, updated" );
								}
								else
								{
									Log( EVerbosityLevel.Verbose, Color.DarkGreen, " ............     hash validated" );
								}
							...
							{
								Log( EVerbosityLevel.Informative, Color.Orange, " ............     hash missing, updated" );
							}
						}
						...
						if( AllFilesUpdated )
						{
							...
							{
								Log( EVerbosityLevel.Informative, Color.Orange, " ............ Found orphaned hashes for the following missing channels:" );
								foreach( string OrphanChannelName in ChannelHashValues.Keys )
								{
									Log( EVerbosityLevel.Informative, Color.Orange, " ............     " + OrphanChannelName );
								}
							...
							Log( EVerbosityLevel.Informative, Color.DarkGreen, "[MaintainCache] Cache hashes validated and up to date" );
						}
					...
					{
						Log( EVerbosityLevel.Verbose, Color.DarkGreen, "[MaintainCache] Performing cache clean up" );
						...
						Log( EVerbosityLevel.Verbose, Color.DarkGreen, "[MaintainCache] Cache clean up complete" );
					}
				}
			}
		}
