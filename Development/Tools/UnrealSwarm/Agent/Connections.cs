...
namespace Agent
{
	...
	public class RemoteConnectionInterfaceWrapper
	{
		...
		public void SignalConnectionDropped()
		{
			...
			{
				AgentApplication.Log( EVerbosityLevel.Verbose, Color.DarkGreen, "[SignalConnectionDropped] Connection dropped for " + RemoteAgentName );
		...
		public void BeginMonitoring()
		{
			...
			AgentApplication.Log( EVerbosityLevel.Verbose, Color.DarkGreen, "[BeginMonitoring] Starting monitoring for " + RemoteAgentName );
		...
		public void EndMonitoring()
		{
			...
			RemoteInterfaceMonitorThread = null;
			AgentApplication.Log( EVerbosityLevel.Verbose, Color.DarkGreen, "[BeginMonitoring] Monitoring ended for " + RemoteAgentName );
		}
		...
		private void RemoteInterfaceMonitorThreadProc()
		{
			try
			{
				...
				while( RemoteInterfaceAlive )
				{
					...
					try
					{
						AgentApplication.Log( EVerbosityLevel.ExtraVerbose, Color.DarkGreen, "[RemoteInterfaceMonitorThreadProc] Pinging " + RemoteAgentName + " at " + RemoteAgentIPAddress + " ..." );
						...
						{
							AgentApplication.Log( EVerbosityLevel.ExtraVerbose, Color.DarkGreen, "[RemoteInterfaceMonitorThreadProc] Testing connection on " + RemoteAgentName + " ..." );
					...
					{
						AgentApplication.Log( EVerbosityLevel.Verbose, Color.Red, "[RemoteInterfaceMonitorThreadProc] Remote connection exception: " + Ex.Message );
					}

					// If we've just dropped the connection, signal it
					if( NewRemoteInterfaceAlive == false )
					{
						AgentApplication.Log( EVerbosityLevel.ExtraVerbose, Color.DarkGreen, "[RemoteInterfaceMonitorThreadProc] " + RemoteAgentName + " has dropped, cleaning up" );
						SignalConnectionDropped();
					}
					else
					{
						// Otherwise, sleep for a bit before trying again
						AgentApplication.Log( EVerbosityLevel.ExtraVerbose, Color.DarkGreen, "[RemoteInterfaceMonitorThreadProc] " + RemoteAgentName + " is still alive" );
						Thread.Sleep( 2000 );
					}
				}
			}
			...
			{
				AgentApplication.Log( EVerbosityLevel.Verbose, Color.DarkGreen, "[RemoteInterfaceMonitorThreadProc] Monitor thread exiting " + RemoteAgentName + " ..." );
			}
		}

		public bool IsAlive()
		{
			AgentApplication.Log( EVerbosityLevel.SuperVerbose, Color.DarkGreen, "[IsAlive] " + RemoteAgentName + "::IsAlive returns " + RemoteInterfaceAlive.ToString() );
	...
	public partial class Agent : MarshalByRefObject, IAgentInternalInterface, IAgentInterface
	{
		...
		public void InitCoordinator()
		{
			try
			{
				...
				Log( EVerbosityLevel.Informative, Color.DarkGreen, " ......... using SwarmCoordinator on " + AgentApplication.Options.CoordinatorRemotingHost );
#if false
			...
			catch( Exception Ex )
			{
				...
				Log( EVerbosityLevel.ExtraVerbose, Color.Orange, "[InitCoordinator] Exception details: " + Ex.ToString() );
			}

			if( ( Coordinator != null ) &&
				( CoordinatorResponding ) )
			{
				Log( EVerbosityLevel.Informative, Color.DarkGreen, " ......... SwarmCoordinator successfully initialized" );
			}
			else
			{
				Log( EVerbosityLevel.Informative, Color.Orange, " ......... SwarmCoordinator failed to be initialized" );
		...
		private Int32 GetUniqueHandle()
		{
				...
				( AgentApplication.Options.EnableStandaloneMode == false ) )
			{
				...
				catch( Exception Ex )
				{
					...
					Log( EVerbosityLevel.Verbose, Color.Orange, "[GetUniqueHandle] Communication with the coordinator failed, job distribution will be disabled until the connection is established" );
					Log( EVerbosityLevel.ExtraVerbose, Color.Orange, "[GetUniqueHandle] Exception: " + Ex.ToString() );
		...
		private Int32 OpenConnection_1_0( Hashtable InParameters, ref Hashtable OutParameters )
		{
			...
			foreach( Connection ExistingConnection in Connections.Values )
			{
				if( ExistingConnection is LocalConnection )
				{
					...
					if( ExistingLocalConnection.ProcessID == LocalProcessID )
					{
						...
						Log( EVerbosityLevel.Informative, Color.Orange, "[Connection] Detected new local connection from same process ID as an existing one, closing the old one" );
			...
			foreach( AgentJob Job in ActiveJobs.Values )
			{
				if( Job.CurrentState == AgentJob.JobState.AGENT_JOB_RUNNING )
				{
					...
					if( ( JobFlags & EJobTaskFlags.FLAG_MANUAL_START ) != 0 )
					{
						...
						{
							if( ProcessFilename == OriginalExecutableName )
							{
								Log( EVerbosityLevel.Informative, Color.DarkGreen, "[Job] Attaching new process handle to existing Job" );
			...
			{
				Log( EVerbosityLevel.Informative, Color.DarkGreen, "[Job] Found a parent connection for PID " + LocalProcessID );
				Log( EVerbosityLevel.Informative, Color.DarkGreen, String.Format( "[Job]     {0:X8} -> {1:X8}", ParentConnection.Handle, NewConnectionHandle ) );
			...
			lock( Connections )
			{
					...
					if( IsAssigned )
					{
						...
						if( Connections.Count != 0 )
						{
							...
							foreach( Connection ExistingConnection in Connections.Values )
							{
								...
								{
									Log( EVerbosityLevel.Informative, Color.Orange, "[Connection] Detected incoming assigned remote connection, closing existing remote connection" );
									Hashtable OldInParameters = null;
									Hashtable OldOutParameters = null;
									CloseConnection( ExistingConnection.Handle, OldInParameters, ref OldOutParameters );
								}
				...
				try
				{
						...
						( AgentApplication.Options.EnableStandaloneMode == false ) )
					{
						...
						if( RequestingAgentInfo != null )
						{
							...
							if( PingRemoteHost( RequestingAgentName, RequestingAgentIPAddress ) )
							{
								...
								Log( EVerbosityLevel.Informative, Color.DarkGreen, "[Connect] Remote agent connection object obtained: " + RequestingAgentName );
								...
								try
								{
									...
									if( RemoteConnectionHandle >= 0 )
									{
										...
										Log( EVerbosityLevel.Informative, Color.DarkGreen, "[Connect] Remote agent connection confirmed: " + RequestingAgentName );
									}
								}
								catch( Exception Ex )
								{
									Log( EVerbosityLevel.Informative, Color.Red, "[Connect] OpenRemoteConnection: " + Ex.ToString() );
							...
							else
							{
								...
								Log( EVerbosityLevel.Informative, Color.Red, "[Connect] Failed to ping " + RequestingAgentName + " at " + RequestingAgentIPAddress + " to confirm the remote connection" );
						...
						else
						{
							...
							Log( EVerbosityLevel.Informative, Color.Red, "[Connect] Failed to lookup " + RequestingAgentName + " in the Coordinator" );
							RemoteConnectionHandle = Constants.ERROR_CONNECTION_NOT_FOUND;
						}
					}
					else
					{
						...
						Log( EVerbosityLevel.Informative, Color.Red, "[Connect] Failed to contact the Coordinator" );
						RemoteConnectionHandle = Constants.ERROR_CONNECTION_NOT_FOUND;
					}
				}
				catch( Exception Ex )
				{
					Log( EVerbosityLevel.Verbose, Color.Red, "[Connect] Remote agent connection failed: " + RequestingAgentName );
					Log( EVerbosityLevel.Verbose, Color.Red, "[Connect] Exception details: " + Ex.ToString() );
				}

				if( RemoteConnectionHandle < 0 )
				{
					// If we get here, we have failed to create the connection
					Log( EVerbosityLevel.Informative, Color.Red, "[Connect] Remote agent connection failed: " + RequestingAgentName );
				}
		...
		private bool PingRemoteHost( string RemoteHostName, string RemoteHostNameOrAddressToPing )
		{
			...
			try
			{
				...
				if( Reply.Status == IPStatus.Success )
				{
					AgentApplication.Log( EVerbosityLevel.SuperVerbose, Color.DarkGreen, "[PingRemoteHost] Successfully pinged " + RemoteHostName + " with " + RemoteHostNameOrAddressToPing );
					PingSuccess = true;
				}
			}
			catch( Exception Ex )
			{
				...
				AgentApplication.Log( EVerbosityLevel.SuperVerbose, Color.DarkGreen, "[PingRemoteHost] Failed to ping " + RemoteHostName + " with " + RemoteHostNameOrAddressToPing );
				AgentApplication.Log( EVerbosityLevel.SuperVerbose, Color.DarkGreen, "[PingRemoteHost] Exception details: " + Ex.Message );
			}
			return PingSuccess;
		}
		...
		public Int32 TryOpenRemoteConnection( LocalConnection ParentConnection, AgentInfo RemoteAgentInfo, bool IsAssigned, DateTime AssignedTimestamp, out RemoteConnection OpenedRemoteConnection )
		{
			...
			Log( EVerbosityLevel.Verbose, Color.DarkGreen, "[Connect] Trying to open a remote connection to " + RemoteAgentName + " at " + RemoteAgentIPAddress );
			...
			if( PingRemoteHost( RemoteAgentName, RemoteAgentIPAddress ) )
			{
				...
				Log( EVerbosityLevel.Verbose, Color.DarkGreen, "[Connect] Remote agent interface object obtained for " + RemoteAgentName + " at " + RemoteAgentIPAddress );
					...
					if( WrappedRemoteAgentInterface.OpenRemoteConnection( Environment.MachineName, NewConnectionHandle, LoggingFlags, IsAssigned, AssignedTimestamp ) >= 0 )
					{
						...
						if( PendingConnections.ContainsKey( NewConnectionHandle ) == false )
						{
							if( Connections.ContainsKey( NewConnectionHandle ) )
							{
								...
								Log( EVerbosityLevel.Informative, Color.DarkGreen, "[Connect] Successfully opened a remote connection with " + RemoteAgentName );
				...
				{
					Log( EVerbosityLevel.Verbose, Color.Red, "[Connect] Exception: " + RemoteAgentName + ": " + Ex.ToString() );
					ErrorCode = Constants.ERROR_EXCEPTION;
				}

				if( ErrorCode < 0 )
				{
					Log( EVerbosityLevel.Verbose, Color.Red, "[Connect] Failed to open a remote connection with " + RemoteAgentName );
				}
		...
		public Int32 CloseConnection_1_0( Int32 ConnectionHandle, Hashtable InParameters, ref Hashtable OutParameters )
		{
			...
			{
				Log( EVerbosityLevel.Informative, Color.DarkGreen, String.Format( "[CloseConnection] Closing connection {0:X8} using handle {1:X8}", ConnectionToClose.Handle, ConnectionHandle ) );
				...
				lock( CloseConnectionLock )
				{
					if( ConnectionToClose.CurrentState == ConnectionState.CONNECTED )
					{
						...
						Log( EVerbosityLevel.Informative, Color.DarkGreen, String.Format( "[CloseConnection] Connection confirmed for disconnection {0:X8}", ConnectionToClose.Handle ) );
						ConnectionToClose.CurrentState = ConnectionState.DISCONNECTING;
						AllowDisconnection = true;
					}
					else
					{
						Log( EVerbosityLevel.Informative, Color.DarkGreen, String.Format( "[CloseConnection] Connection already being disconnected, nothing to do {0:X8}", ConnectionToClose.Handle ) );
					}
				}
				...
				if( AllowDisconnection )
				{
					...
					if( ConnectionToClose.Job != null )
					{
						...
						if( ConnectionToClose.Job.Owner == ConnectionToClose )
						{
							...
							{
								Log( EVerbosityLevel.Informative, Color.Orange, String.Format( "[CloseConnection] Closing orphaned Job ({0})", ConnectionToClose.Job.JobGuid ) );
							}
					...
					{
						Log( EVerbosityLevel.Informative, Color.DarkGreen, String.Format( "[CloseConnection] Orphaning local connection ({0:X8} is the parent of {1:X8})", ConnectionToClose.Handle, LocalChild.Handle ) );
					...
					foreach( RemoteConnection RemoteChild in ConnectionToClose.RemoteChildren.Values )
					{
						...
						Log( EVerbosityLevel.Informative, Color.DarkGreen, String.Format( "[CloseConnection] Recursively closing remote connection ({0:X8} is the parent of {1:X8})", ConnectionToClose.Handle, RemoteChild.Handle ) );
					...
					else
					{
						...
						Log( EVerbosityLevel.Informative, Color.DarkGreen, String.Format( "[CloseConnection] Closing bi-directional remote connection ({0:X8})", RemoteConnectionToClose.Handle ) );
						...
						Log( EVerbosityLevel.Verbose, Color.DarkGreen, String.Format( "[CloseConnection] Bi-directional remote connection closed ({0:X8})", RemoteConnectionToClose.Handle ) );
					}
					...
					if( FlushMessageQueue( ConnectionToClose, true ) == false )
					{
						...
						Log( EVerbosityLevel.Informative, Color.DarkGreen, String.Format( "[CloseConnection] Fallback path: Connection disconnected {0:X8}", ConnectionToClose.Handle ) );
			...
			{
				Log( EVerbosityLevel.Critical, Color.Red, String.Format( "[CloseConnection] Rejected, unrecognized or inactive connection ({0:X8})", ConnectionHandle ) );
				ErrorCode = Constants.ERROR_CONNECTION_NOT_FOUND;
			}
		...
		public bool ResetPotentialRemoteAgents( LocalConnection ParentConnection )
		{
			...
			{
				Log( EVerbosityLevel.Complex, Color.DarkBlue, String.Format( "[Connect] Coordinator named remote agent {0} which is {1}", NextAgent.Name, NextAgent.State ) );
			}
		...
		private void PrintSimpleAgentList( List<AgentInfo> AgentInfoList )
		{
			...
			{
				Log( EVerbosityLevel.Informative, Color.DarkGreen, NextMessage );
			}
		}
		...
		public void RestartAgentGroupThreadProc( Object MethodData )
		{
			...
			try
			{
					...
					( AgentApplication.Options.EnableStandaloneMode == false ) )
				{
					...
					Log( EVerbosityLevel.Informative, Color.DarkGreen, "[Network] Restarting the following agents (in group " + AgentGroupName + ")" );
					PrintSimpleAgentList( RemoteAgentsInGroupBefore );

					// Tell the coordinator to restart them
					Log( EVerbosityLevel.Informative, Color.DarkGreen, "[Network] Restarting now" );
					Coordinator.RestartAgentGroup( AgentGroupName );

					Log( EVerbosityLevel.Informative, Color.DarkGreen, "[Network] Waiting as agents come back online..." );
					...
					AgentApplication.Log( EVerbosityLevel.Informative, Color.DarkGreen, "[Network] Requested agent restart complete" );
				}
			}
			catch( Exception )
			{
			...
			if( ShouldPing )
			{
				...
				if( CouldPing )
				{
					...
					try
					{
						...
						{
							Log( EVerbosityLevel.Informative, Color.DarkGreen, "[PingCoordinator] Restart has been requested" );
							...
							{
								Log( EVerbosityLevel.Informative, Color.DarkGreen, "[PingCoordinator] Restart request ignored for non-published agents" );
							}
						}
					...
					catch( Exception Ex )
					{
						...
						Log( EVerbosityLevel.Informative, Color.Orange, "[Ping] Communication with the coordinator failed, job distribution will be disabled until the connection is established" );
						Log( EVerbosityLevel.ExtraVerbose, Color.Orange, "Exception details: " + Ex.ToString() );
				...
				{
					AgentApplication.Log( EVerbosityLevel.ExtraVerbose, Color.DarkGreen, "[PingCoordinator] Determined that we couldn't ping the coordinator" );
					if( Coordinator == null )
					{
						AgentApplication.Log( EVerbosityLevel.ExtraVerbose, Color.DarkGreen, "[PingCoordinator] Coordinator is null" );
					}
					else if( CoordinatorResponding == false )
					{
						AgentApplication.Log( EVerbosityLevel.ExtraVerbose, Color.DarkGreen, "[PingCoordinator] CoordinatorResponding is false" );
					}
					else
					{
						AgentApplication.Log( EVerbosityLevel.ExtraVerbose, Color.DarkGreen, "[PingCoordinator] Coordinator ping failed" );
					}
				}
			}
			else
			{
				AgentApplication.Log( EVerbosityLevel.SuperVerbose, Color.DarkGreen, "[PingCoordinator] Determined that we shouldn't ping the coordinator right now" );
			}
		...
		public void RequestShutdown()
		{
			...
			Log( EVerbosityLevel.Informative, Color.DarkGreen, "[RequestShutdown] Shutdown has been requested" );
		...
		public void MaintainConnections()
		{
			...
			foreach( KeyValuePair<Int32, Connection> NextHandleAndConnection in Connections.Copy() )
			{
				...
				Log( EVerbosityLevel.SuperVerbose, Color.DarkGreen, String.Format( "[MaintainConnections] Maintaining Connection ({0:X8})", NextHandle ) );
				...
				if( NextConnection.CurrentState == ConnectionState.CONNECTED )
				{
					if( NextConnection is LocalConnection )
					{
						...
						{
							Log( EVerbosityLevel.Informative, Color.DarkGreen, String.Format( "[MaintainConnections] Detected dropped local connection, cleaning up ({0:X8})", Local.Handle ) );
							DroppedConnections.Add( Local );
						}
						else
						{
							Log( EVerbosityLevel.SuperVerbose, Color.DarkGreen, String.Format( "[MaintainConnections] Local connection alive ({0:X8})", Local.Handle ) );
						}
					}
					else
					{
						...
						{
							Log( EVerbosityLevel.Informative, Color.DarkGreen, String.Format( "[MaintainConnections] Detected dropped remote connection, cleaning up ({0:X8})", Remote.Handle ) );
						...
						else
						{
							...
							{
								Log( EVerbosityLevel.Informative, Color.DarkGreen, String.Format( "[MaintainConnections] Detected finished connection, closing ({0:X8})", Remote.Handle ) );
								Hashtable RemoteInParameters = null;
								Hashtable RemoteOutParameters = null;
							...
							{
								Log( EVerbosityLevel.SuperVerbose, Color.DarkGreen, String.Format( "[MaintainConnections] Remote connection alive ({0:X8})", Remote.Handle ) );
							}
						}
					}
				}
				...
				else if( NextConnection.CurrentState == ConnectionState.DISCONNECTED )
				{
					if( NextConnection is LocalConnection )
					{
						...
						{
							Log( EVerbosityLevel.Informative, Color.DarkGreen, String.Format( "[MaintainConnections] Local connection has closed ({0:X8})", Local.Handle ) );
						...
						else
						{
							...
							if( ( TimeSinceConnectionClosed > TimeToWait ) &&
								( Local.Job == null ) )
							{
								Log( EVerbosityLevel.Informative, Color.DarkGreen, String.Format( "[MaintainConnections] Local connection has timed-out and is being forceably closed ({0:X8})", Local.Handle ) );
								ClosedAndInactiveConnections.Add( Local );
							}
							else
							{
								Log( EVerbosityLevel.ExtraVerbose, Color.DarkGreen, String.Format( "[MaintainConnections] Waiting for closed local conection to drain its message queue... ({0:X8})", Local.Handle ) );
							}
						}
					}
					else
					{
						...
						Log( EVerbosityLevel.Informative, Color.DarkGreen, String.Format( "[MaintainConnections] Remote connection has closed ({0:X8})", NextHandle ) );
						ClosedAndInactiveConnections.Add( NextConnection );
					}
				}
				else
				{
					Log( EVerbosityLevel.Informative, Color.DarkGreen, String.Format( "[MaintainConnections] Connection {0:X8} is {1}", NextHandle, NextConnection.CurrentState ) );
				}
			}
			...
			foreach( Connection Closed in ClosedAndInactiveConnections )
			{
				...
				foreach( Channel OpenChannel in Closed.OpenChannels.Values )
				{
					...
					{
						Log( EVerbosityLevel.Informative, Color.Orange, "[MaintainConnections] Closing orphaned channel: \"" + OpenChannel.Name + "\"" );
						...
						if( File.Exists( OpenChannel.FullName ) )
						{
							...
							catch( Exception )
							{
								...
								{
									Log( EVerbosityLevel.Informative, Color.Orange, "[MaintainConnections] Failed to delete orphaned, writable Job channel (generally benign): \"" + OpenChannel.Name + "\"" );
								}
								else
								{
									Log( EVerbosityLevel.Informative, Color.Orange, "[MaintainConnections] Failed to delete orphaned, writable persistent cache channel (possible cache corruption): \"" + OpenChannel.Name + "\"" );
								}
							}
						}
					}
					...
					if( Closed is RemoteConnection )
					{
						...
						foreach( Int32 Alias in RemoteClosed.Aliases )
						{
							...
							string LogMessage = String.Format( "[MaintainConnections] Removed alias for remote connection: {0:X8} was an alias for {1:X8}",
								Alias,
								RemoteClosed.Handle );
							Log( EVerbosityLevel.Informative, Color.DarkGreen, LogMessage );
						}
					}

					Log( EVerbosityLevel.Informative, Color.DarkGreen, String.Format( "[MaintainConnections] Removed connection {0:X8}", Closed.Handle ) );
				}

				// If this is the last connection open, log it out and reset the NextCleanUpCacheTime
				if( Connections.Count == 0 )
				{
					Log( EVerbosityLevel.Informative, Color.DarkGreen, "[MaintainConnections] All connections have closed" );
