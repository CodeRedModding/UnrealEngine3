	...
	public partial class Agent : MarshalByRefObject, IAgentInternalInterface, IAgentInterface
	{
		...
		private Int32 SendMessage_1_0( Int32 ConnectionHandle, Hashtable InParameters, ref Hashtable OutParameters )
		{
			...
			else
			{
				...
				Log( EVerbosityLevel.Verbose, Color.Orange, NewLogMessage );
		...
		public Int32 SendMessageInternal( Connection Sender, AgentMessage NewMessage )
		{
			...
			if( NewMessage.To == Constants.INVALID )
			{
				...
				if( NewMessage.Version == ESwarmVersionValue.VER_1_0 )
				{
					...
					switch( NewMessage.Type )
					{
						...
						default:
                            Log( EVerbosityLevel.Informative, Color.Orange, "SendMessage: Invalid message type received, ignoring " + NewMessage.Type.ToString() );
							break;
					}
					...
					if( NewMessage.To == Constants.INVALID )
					{
                        Log( EVerbosityLevel.Informative, Color.Orange, "SendMessage: No proper recipient found, ignoring " + NewMessage.Type.ToString() );
			...
			if( bMessageIsValid )
			{
				lock( MessageQueueLock )
				{
					...
					Log( EVerbosityLevel.SuperVerbose, Color.DarkGreen, NewLogMessage );
			...
			{
				Log( EVerbosityLevel.Informative, Color.Orange, String.Format( "SendMessage: Discarded message \"{0}\"", NewMessage.Type ) );
			}
		...
		{
			Log( EVerbosityLevel.ExtraVerbose, Color.DarkGreen, String.Format( "[FlushMessageQueue] Draining message queue for {0:X8}", RequestingConnection.Handle ) );
			...
			Log( EVerbosityLevel.ExtraVerbose, Color.DarkGreen, String.Format( "[FlushMessageQueue] Lock acquired for {0:X8}", RequestingConnection.Handle ) );
			...
			Log( EVerbosityLevel.ExtraVerbose, Color.DarkGreen, String.Format( "[FlushMessageQueue] Lock released for {0:X8}", RequestingConnection.Handle ) );

			if( SignalMessageSent == Constants.SUCCESS )
			{
				SignalMessage.ResetEvent.WaitOne( Timeout.Infinite );
				Log( EVerbosityLevel.ExtraVerbose, Color.DarkGreen, String.Format( "[FlushMessageQueue] Drain complete for {0:X8}", RequestingConnection.Handle ) );
				return true;
			}
			else
			{
				Log( EVerbosityLevel.ExtraVerbose, Color.DarkGreen, "[FlushMessageQueue] Drain failed because of an error sending the signal message" );
				return false;
			}
		}
			...
			while( AgentHasShutDown == false )
			{
				...
				while( MessageQueuePM.Count > 0 )
				{
					...
					switch( NextMessage.Type )
					{
						case EMessageType.SIGNAL:
						{
							if( NextMessage is DisconnectionSignalMessage )
							{
								...
								Log( EVerbosityLevel.Informative, Color.DarkGreen, String.Format( "[CloseConnection] Connection disconnected {0:X8}", DisconnectMessage.ConnectionToDisconnect.Handle ) );
						...
						case EMessageType.TASK_REQUEST:
						{
							...
							if( Connections.TryGetValue( NextMessage.From, out RequestingConnection ) )
							{
								...
								else
								{
									...
									Log( EVerbosityLevel.Verbose, Color.Orange, "[ProcessMessage] Unable to find Job for Task Request; may have been closed" );
							...
							else
							{
								...
								Log( EVerbosityLevel.Verbose, Color.Orange, "[ProcessMessage] Unable to find owning Connection for Task Request" );
								bMessageHandled = true;
							}
						}
						break;
						...
						case EMessageType.TASK_STATE:
						{
								...
								( SendingConnection.Job != null ) )
							{
								...
								else
								{
									...
									Log( EVerbosityLevel.Verbose, Color.Orange, "[ProcessMessage] Unable to find Job for Task Request" );
									bMessageHandled = true;
								}
							}
							else
					...
					if( bMessageHandled == false )
					{
						...
						{
                            Log( EVerbosityLevel.Informative, Color.Orange, "ProcessMessage: Message sent to invalid connection, ignoring: " + NextMessage.Type.ToString() );
						}
					}
		...
		private Int32 GetMessage_1_0( Int32 ConnectionHandle, Hashtable InParameters, ref Hashtable OutParameters )
		{
			...
			{	
				...
				else
				{
					...
					Log( EVerbosityLevel.Verbose, Color.DarkGreen, String.Format( "[GetMessage] Message available timeout expired, safely returning to {0:X8} with no message", ConnectionHandle ) );
					ErrorCode = Constants.SUCCESS;
				}
			}
			else
			{
				...
				Log( EVerbosityLevel.Informative, Color.Red, LogMessage );
                ErrorCode = Constants.ERROR_CONNECTION_NOT_FOUND;
			}
			...
			else if( Timeout == -1 )
			{
				...
				{
					Log( EVerbosityLevel.Informative, Color.DarkGreen, String.Format( "[GetMessage] Safely returning to {0:X8} with no message", ConnectionHandle ) );
				}
				else
				{
					Log( EVerbosityLevel.Informative, Color.Orange, String.Format( "[GetMessage] Returning to {0:X8} with no message, possibly in error", ConnectionHandle ) );
				}
			}
