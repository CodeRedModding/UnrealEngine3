...
namespace Agent
{
	...
	public class AgentJob
	{
		...
		private bool RequestDependency( Connection RequestingConnection, string Dependency, bool bIsRequired )
		{
			...
			if( Manager.TestChannel( RequestingConnection.Handle, InParameters, ref OutParameters ) < 0 )
			{
				if( RequestingConnection is RemoteConnection )
				{
					...
					Manager.Log( EVerbosityLevel.Verbose, Color.DarkGreen, "[Job] Attempting to pull file from remote Agent: " + Dependency );
					if( Manager.PullChannel( RequestingConnection as RemoteConnection, Dependency, null ) == false )
					{
						if( bIsRequired )
						{
							Manager.Log( EVerbosityLevel.Informative, Color.Red, "[Job] Error: Failed to pull a required file from remote Agent: " + Dependency );
							return false;
						}
						else
						{
							Manager.Log( EVerbosityLevel.Verbose, Color.Orange, "[Job] Warning: Failed to pull an optional file from remote Agent: " + Dependency );
						}
					}
				}
				else
				{
					...
					if( bIsRequired )
					{
						...
						Manager.Log( EVerbosityLevel.Informative, Color.Red, "[Job] Error: Failed to find required file: " + Dependency );
						return false;
					}
					else
					{
						Manager.Log( EVerbosityLevel.Verbose, Color.Orange, "[Job] Warning: Failed to find an optional file: " + Dependency );
					}
				}
			}
			return true;
		...
		public Int32 BeginJobSpecification( AgentJobSpecification NewJobSpecification32, Hashtable NewJobDescription32, AgentJobSpecification NewJobSpecification64, Hashtable NewJobDescription64 )
		{
			...
			{
				Manager.Log( EVerbosityLevel.Critical, Color.Red, "[Job] BeginJobSpecification: Failed to cache necessary job files" );
		...
		public Int32 AddTask( AgentTaskSpecification NewTaskSpecification )
		{
			...
			if( RequestTaskFiles( Owner, NewTaskSpecification ) )
			{
				lock( PendingTasks )
				{
					Manager.Log( EVerbosityLevel.Verbose, Color.DarkGreen, "[Job] AddTask: Adding \"" + NewTaskSpecification.Parameters + "\" (" + NewTaskSpecification.TaskGuid.ToString() + ") with cost " + NewTaskSpecification.Cost.ToString() );
			...
			{
				Manager.Log( EVerbosityLevel.Critical, Color.Red, "[Job] AddTask: Failed to cache necessary task files" );
				ErrorCode = Constants.ERROR_CHANNEL_NOT_FOUND;
			}
		...
		private void SendJobCompletedMessage( AgentInfoMessage AdditionalInfoMessage )
		{
			...
			{
				Manager.Log( EVerbosityLevel.Informative, Color.DarkGreen, "[Job] " + AdditionalInfoMessage.TextMessage );
			...
			{
				Manager.Log( EVerbosityLevel.Informative, Color.Red, "[Job] " + AdditionalInfoMessage.TextMessage );
		...
		public void OutputReceivedDataEventHandler( Object Sender, DataReceivedEventArgs Line )
		{
				...
				( Line.Data != null ) )
			{
				...
				{
					Manager.Log( EVerbosityLevel.Informative, Color.MediumBlue, Line.Data );
				}
				else
				{
					if( ProcessObjectOutputLines == AgentApplication.DeveloperOptions.MaximumJobApplicationLogLines )
					{
						Manager.Log( EVerbosityLevel.Informative, Color.MediumBlue, "Maximum output from job application received (defined in the Agent Settings)" );
						Manager.Log( EVerbosityLevel.Informative, Color.MediumBlue, "Remaining output from job application truncated - see the job log for complete output" );
					}
			
					// Send it along anyway, but mark it as the highest level of verbosity
					Manager.Log( EVerbosityLevel.ExtraVerbose, Color.MediumBlue, Line.Data );
				}
		...
		public void ErrorReceivedDataEventHandler( Object Sender, DataReceivedEventArgs Line )
		{
				...
				( Line.Data != null ) )
			{
				if( ProcessObjectOutputLines < AgentApplication.DeveloperOptions.MaximumJobApplicationLogLines )
				{
					Manager.Log( EVerbosityLevel.Informative, Color.Red, Line.Data );
				}
				else
				{
					if( ProcessObjectOutputLines == AgentApplication.DeveloperOptions.MaximumJobApplicationLogLines )
					...
					// Send it along anyway, but mark it as the highest level of verbosity
					Manager.Log( EVerbosityLevel.ExtraVerbose, Color.Red, Line.Data );
				}
				ProcessObjectOutputLines++;
			}
		}
		...
		private void PostJobStatsToDB()
		{
			try
			{
				...
				lock( PostJobStatsToDBLock )
				{
						...
						( Properties.Settings.Default.StatsDatabaseName.Length > 0 ) )
					{
						...
						Manager.Log( EVerbosityLevel.Verbose, Color.DarkGreen, "[PostJobStatsToDB] Posting stats to DB" );
						Manager.Log( EVerbosityLevel.Verbose, Color.DarkGreen, "[PostJobStatsToDB] " + SqlCommandString );
			...
			{
				Manager.Log( EVerbosityLevel.Verbose, Color.Orange, "[PostJobStatsToDB] Database error:" );
				Manager.Log( EVerbosityLevel.Verbose, Color.Orange, "[PostJobStatsToDB] " + Ex.Message );
			}
		}
		...
		private Int32 StartJobExecution()
		{
			...
			try
			{
				...
				foreach( KeyValuePair<string, string> Pair in Specification.DependenciesOriginalNames )
				{
					...
					{
						Manager.Log( EVerbosityLevel.Informative, Color.Red, "[Job] Failed to use file \"" + Pair.Key + "\" because of a security violation" );
					}
				}
				...
				else
				{
					...
					if( Specification.DependenciesOriginalNames.TryGetValue( Specification.ExecutableName, out OriginalExecutableName ) )
					{
						...
						if( TaskProcess.Start() )
						{
							...
							Manager.Log( EVerbosityLevel.Informative, Color.DarkGreen, "[Job] Launched Job " + Specification.ExecutableName );
							Manager.Log( EVerbosityLevel.Informative, Color.DarkGreen, "[Job]     PID is " + ProcessObject.Id.ToString() );
							Manager.Log( EVerbosityLevel.Informative, Color.DarkGreen, "[Job]     GUID is \"" + JobGuid.ToString() + "\"" );
					...
					( CurrentState == JobState.AGENT_JOB_RUNNING ) )
				{
					...
					if( AgentApplication.Options.BringToFront )
					{
			...
			catch( Exception Ex )
			{
				...
				Manager.Log( EVerbosityLevel.Critical, Color.Red, "[StartJobExecution] Exception was: " + Ex.ToString() );
			...
			if( MethodData != null )
			{
				...
				if( JobErrorCode >= 0 )
				{
					...
					if( JobErrorCode >= 0 )
					{
						...
						if( JobErrorCode >= 0 )
						{
							Job.Manager.Log( EVerbosityLevel.Informative, Color.DarkGreen, "[EndJobSpecification] Started job on remote connection" );
						}
					}
				}
				if( JobErrorCode < 0 )
				{
					Job.Manager.Log( EVerbosityLevel.Informative, Color.Orange, "[EndJobSpecification] Tried to begin the job on a valid remote connection, but failed" );
		...
		private bool ValidateDeterministicDistributionRequirements()
		{
			...
			if( Specification.DependenciesOriginalNames.TryGetValue( Specification.ExecutableName, out ThisOriginalExecutableName ) )
			{
				...
				if( LastSuccessfulJobRecord.Specification.DependenciesOriginalNames.TryGetValue( LastSuccessfulJobRecord.Specification.ExecutableName, out LastOriginalExecutableName ) )
				{
					...
					{
						Manager.Log( EVerbosityLevel.Critical, Color.Red, "[Deterministic] Failed to validate Job, executable name is different" );
					}
				}
			}
			...
			if( ExecutableNameMatches )
			{
					...
					foreach( AgentTask Task in ListOfTasks )
					{
						if( LastSuccessfulJobRecord.AllTasks.TryGetValue( Task.Specification.TaskGuid, out NextTask ) )
						{
							...
							else
							{
								...
								Manager.Log( EVerbosityLevel.Critical, Color.Red, "[Deterministic] Failed to validate Job, task is different" );
								Manager.Log( EVerbosityLevel.Critical, Color.Red, OldTaskDetails );
								Manager.Log( EVerbosityLevel.Critical, Color.Red, NewTaskDetails );
							}
						}
					}
				...
				{
					Manager.Log( EVerbosityLevel.Critical, Color.Red, "[Deterministic] Failed to validate Job, overall task count is different" );
				}
				...
				if( AllTasksMatch )
				{
					...
					for( Int32 i = 0; i < LastSuccessfulJobRecord.WorkerAgentNames.Count; i++ )
					{
							...
							( Manager.AgentNamePassesAllowedAgentsFilter( WorkerAgentName ) ) )
						{
							...
							{
								Manager.Log( EVerbosityLevel.Critical, Color.Red, "[Deterministic] Could not acquire connection to " + WorkerAgentName + " necessary for job" );
							}
						}
						else
						{
					...
					if( WorkerAgentsEmployed == LastSuccessfulJobRecord.WorkerAgentNames.Count )
					{
						...
						// we can -> success
						Manager.Log( EVerbosityLevel.Informative, Color.DarkGreen, "[Deterministic] Job validated, deterministic distribution enabled" );
						DeterministicModeValidated = true;
					}
					else
					{
						Manager.Log( EVerbosityLevel.Critical, Color.Red, "[Deterministic] Failed to validate Job, could not acquire all agents necessary" );
					}
				}
			}
					...
					if( OwnerIsInstigator )
					{
						...
						foreach( AgentTask NextTask in RetiredTasks.ToArray() )
						{
							if( NextTask.CurrentState.TaskState == EJobTaskState.TASK_STATE_COMPLETE_SUCCESS )
							{
								...
								Manager.Log( EVerbosityLevel.Verbose, Color.DarkGreen, LogMessage );
							}
						}
					}
				}
			...
			if( RunningTasks.TryGetValue( UpdatedTaskState.TaskGuid, out RunningTask ) )
			{
				...
				switch( UpdatedTaskState.TaskState )
				{
					...
					case EJobTaskState.TASK_STATE_REJECTED:
						if( RunningTask.CurrentOwner is RemoteConnection )
						{
							Manager.Log( EVerbosityLevel.Informative, Color.Orange, "[UpdateTaskState]: Task Rejected remotely by " + ( RunningTask.CurrentOwner as RemoteConnection ).Info.Name );
							Manager.Log( EVerbosityLevel.Informative, Color.Orange, "[UpdateTaskState]: Requeueing: " + RunningTask.Specification.Parameters );
							UpdateTaskStateAsRequeued( RunningTask );
						}
						else
						{
							Manager.Log( EVerbosityLevel.Informative, Color.Red, "[UpdateTaskState]: Task Rejected locally by " + Environment.MachineName + ", counted as failure");
					...
					case EJobTaskState.TASK_STATE_KILLED:
						if( RunningTask.CurrentOwner is RemoteConnection )
						{
							Manager.Log( EVerbosityLevel.Informative, Color.Orange, "[UpdateTaskState]: Task Killed remotely by " + ( RunningTask.CurrentOwner as RemoteConnection ).Info.Name );
							Manager.Log( EVerbosityLevel.Informative, Color.Orange, "[UpdateTaskState]: Requeueing: " + RunningTask.Specification.Parameters );
							UpdateTaskStateAsRequeued( RunningTask );
						}
						else
						{
							Manager.Log( EVerbosityLevel.Informative, Color.Red, "[UpdateTaskState]: Task Killed locally by " + Environment.MachineName + ", counted as failure" );
							UpdateTaskStateAsFailure( RunningTask );
						}
						break;
					...
					case EJobTaskState.TASK_STATE_COMPLETE_FAILURE:
						if( RunningTask.CurrentOwner is RemoteConnection )
						{
							Manager.Log( EVerbosityLevel.Informative, Color.Red, "[UpdateTaskState]: Task Failed on " + ( RunningTask.CurrentOwner as RemoteConnection ).Info.Name );
						}
						else
						{
							Manager.Log( EVerbosityLevel.Informative, Color.Red, "[UpdateTaskState]: Task Failed on " + Environment.MachineName );
						}
						Manager.Log( EVerbosityLevel.Informative, Color.Red, "[UpdateTaskState]: Task Failed: " + RunningTask.Specification.Parameters );
						UpdateTaskStateAsFailure( RunningTask );
						break;
				}
	...
	public partial class Agent : MarshalByRefObject, IAgentInternalInterface, IAgentInterface
	{
			...
			if( Connections.TryGetValue( ConnectionHandle, out JobOwner ) )
			{
				...
				if( ActiveJobs.Count > 0 )
				{
					foreach( AgentJob Job in ActiveJobs.Values )
					{
						...
						if( Job.Owner is RemoteConnection )
						{
							...
							Log( EVerbosityLevel.Verbose, Color.DarkGreen, "[OpenJob] Interrupting remote job in favor of local one" );
				...
				if( ErrorCode >= 0 )
				{
					...
					if( ErrorCode >= 0 )
					{
						...
						Log( EVerbosityLevel.Informative, Color.DarkGreen, "[Job] Accepted Job " + JobGuid.ToString() );
						ErrorCode = Constants.SUCCESS;
					}
				}
				...
				if( ErrorCode < 0 )
				{
					Log( EVerbosityLevel.Critical, Color.Red, "[Job] OpenJob: Rejected, unable to create necessary directories" );
					ErrorCode = Constants.ERROR_JOB;
				}
			}
			else
			{
				Log( EVerbosityLevel.Critical, Color.Red, "[Job] OpenJob: Rejected, unrecognized connection" );
		...
		private Int32 BeginJobSpecification_1_0( Int32 ConnectionHandle, Hashtable InParameters, ref Hashtable OutParameters )
		{
			...
			{
				Log( EVerbosityLevel.Critical, Color.Red, "[Job] BeginJobSpecification: Rejected, unrecognized connection or job" );
				ErrorCode = Constants.ERROR_CONNECTION_NOT_FOUND;
			}
		...
		private Int32 AddTask_1_0( Int32 ConnectionHandle, Hashtable InParameters, ref Hashtable OutParameters )
		{
				...
				( JobOwner.Job != null ) ) )
			{
				...
				{
					Log( EVerbosityLevel.Critical, Color.Red, "[Job] AddTask: Rejected, cannot add a task to a running job" );
					ErrorCode = Constants.ERROR_JOB;
				}
			}
			else
			{
				Log( EVerbosityLevel.Critical, Color.Red, "[Job] AddTask: Rejected, unrecognized connection or job" );
				ErrorCode = Constants.ERROR_CONNECTION_NOT_FOUND;
			}
		...
		private Int32 EndJobSpecification_1_0( Int32 ConnectionHandle, Hashtable InParameters, ref Hashtable OutParameters )
		{
			...
			{
				Log( EVerbosityLevel.Critical, Color.Red, "[Job] EndJobSpecification: Rejected, unrecognized connection or job" );
				ErrorCode = Constants.ERROR_CONNECTION_NOT_FOUND;
			}
		...
		private Int32 CloseJob_1_0( Int32 ConnectionHandle, Hashtable InParameters, ref Hashtable OutParameters )
		{
			...
			{
				Log( EVerbosityLevel.Critical, Color.Red, "[Job] CloseJob: Rejected, unrecognized connection or job" );
				ErrorCode = Constants.ERROR_CONNECTION_NOT_FOUND;
			}
		...
		public Int32 GetActiveJobGuid( Int32 ConnectionHandle, ref AgentGuid ActiveJobGuid )
		{
			...
			{
				Log( EVerbosityLevel.Critical, Color.Red, "[Job] GetActiveJobGuid: Rejected, unrecognized connection" );
				ErrorCode = Constants.ERROR_CONNECTION_NOT_FOUND;
			}
					...
					lock( Job.ProcessObjectLock )
					{
						...
						if( Job.CurrentState == AgentJob.JobState.AGENT_JOB_RUNNING )
						{
							...
							if( Job.ProcessObject == null )
							{
								...
								{
									Log( EVerbosityLevel.Informative, Color.Orange, "[Maintain Jobs] Job \"" + WaitingJobName + "\" is not connected yet" );
								}
								else
								{
									Log( EVerbosityLevel.ExtraVerbose, Color.Orange, "[Maintain Jobs] Job \"" + WaitingJobName + "\" is either trying to start up or shut down" );
								}
							}
							else
							{
								...
								Log( EVerbosityLevel.SuperVerbose, Color.DarkGreen, "[Maintain Jobs] Job \"" + JobName + "\" has been running for " + JobRunningTime.TotalSeconds.ToString() + " seconds" );
						...
						else if( Job.CurrentState == AgentJob.JobState.AGENT_JOB_CLOSED )
						{
								...
								if( AgentApplication.DeveloperOptions.JobExecutableTimeout >= 0 )
								{
									...
									if( TimeSinceJobStopped > TimeToWait )
									{
										string NewLogMessage = String.Format( "[Maintain Jobs] Waited for rogue Job for {0} seconds: \"{1}\"",
											...
											Job.JobGuid );
										Log( EVerbosityLevel.Informative, Color.Orange, NewLogMessage );

										WaitedLongEnough = true;
									}
									else
									{
										Log( EVerbosityLevel.ExtraVerbose, Color.Orange, "[Maintain Jobs] Waiting for Job to quit before killing: \"" + Job.JobGuid + "\"" );
									}
								}
							...
							else
							{
								...
								Log( EVerbosityLevel.Verbose, Color.DarkGreen, "[Maintain Jobs] Job is closed and done, removing from list: \"" + Job.JobGuid + "\"" );
								InactiveJobs.Add( Job.JobGuid );
							}
						}
						else
						{
							Log( EVerbosityLevel.ExtraVerbose, Color.Orange, "[Maintain Jobs] Job is " + Job.CurrentState.ToString() );
						}
					}
				}
			}
