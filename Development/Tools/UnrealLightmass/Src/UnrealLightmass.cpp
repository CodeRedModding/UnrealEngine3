...
FString CompareLightingFiles(const TCHAR* Filename1, const TCHAR* Filename2, FLOAT ErrorThreshold)
{
	...
	HANDLE File1 = CreateFileW(Filename1, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	if (File1 == INVALID_HANDLE_VALUE)
	{
		return FString::Printf(TEXT("File '%s' does not exist!"), Filename1);
	}

	HANDLE File2 = CreateFileW(Filename2, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
...
void CompareLightingResults(const TCHAR* Dir1, const TCHAR* Dir2, FLOAT ErrorThreshold)
{
	...
	intptr_t Find = _wfindfirst(*FString::Printf(TEXT("%s\\*.bin"), Dir1), &FindData);
	if (Find != -1L)
	{
		do
		{
		...
		while (_wfindnext(Find, &FindData) == 0);
		_findclose(Find);
	}
...
void CriticalErrorCallback()
{
	...
	if ( GSwarm )
	{
		GSwarm->SendTextMessage( TEXT("*** CRITICAL ERROR! Machine: %s"), appComputerName() );
		GSwarm->SendTextMessage( TEXT("*** CRITICAL ERROR! Logfile: %s"), GLog->GetLogFilename() );
		GSwarm->SendTextMessage( TEXT("*** CRITICAL ERROR! Crash report: %s"), *CrashReporterURL );
		GSwarm->ReportFile( GLog->GetLogFilename() );
		delete GSwarm;
		GSwarm = NULL;
	}
	else
