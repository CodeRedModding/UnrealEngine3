...
#define LM_COMPRESS_OUTPUT_DATA 1
	...
	struct FLightSampleData
	{
		...
		{
			return FColor(
				(BYTE)Clamp<INT>(Coefficients[Component][0] * 255, 0, 255), 
				(BYTE)Clamp<INT>(Coefficients[Component][1] * 255, 0, 255), 
				(BYTE)Clamp<INT>(Coefficients[Component][2] * 255, 0, 255),
				0);
		}
	};
	...
	struct FShadowSampleData
	{
		...
		FColor GetColor(INT Component) const
		{
			BYTE Gray = (BYTE)Clamp<INT>(Visibility * 255, 0, 255);
