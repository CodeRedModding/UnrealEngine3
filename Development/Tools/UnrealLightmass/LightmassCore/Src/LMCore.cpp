...
FLogInternal::FLogInternal()
{
	...
	File = CreateFileW(*LogName, GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
	...
	if (File != INVALID_HANDLE_VALUE)
	{
		...
		::DWORD BytesWritten;
		WriteFile(File, &UnicodeBOM, 2, &BytesWritten, NULL);
	}
	else
	{
...
void FLogInternal::Log( const TCHAR* Text )
{
	...
	{
		::DWORD BytesWritten;
		WriteFile(File, Text, appStrlen(Text) * sizeof(TCHAR), &BytesWritten, NULL);
