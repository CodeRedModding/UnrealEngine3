...
namespace NetworkProfiler
{
	...
	class NetworkStream
	{
		...
		public int GetClassNameIndex(int ActorNameIndex)
		{
			...
			string ClassName = ActorName.Substring(0,ActorName.LastIndexOf('_'));
		 ...
		 */
		public void UpdateSummary( ref Dictionary<int,TypeSummary> Summaries, int NameIndex, int SizeBits )
		{
			...
			{
				Summaries.Add( NameIndex, new TypeSummary( SizeBits ) );
			}
		}
	}
	...
	class TypeSummary
	{
		...
		/** Constructor */
		public TypeSummary( long InSizeBits )
		{
			SizeBits = InSizeBits;
		}
	}
