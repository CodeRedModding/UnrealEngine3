...
namespace RPCUtility
{
	public class CommandHelper
	{
			...
			foreach (string Pair in CopyCommands)
			{
				...
				if (Filenames.Length == 2)
				{
					if (File.Exists(Filenames[0]))
					{
						...
						// store local time
						Int64 LocalTime = ToRemoteTime(File.GetLastWriteTimeUtc(Filenames[0]));
		...
		{
			// remote time is seconds since 2001, we need ticks since 1
			Int64 SecondsSince2001 = RemoteTime;
