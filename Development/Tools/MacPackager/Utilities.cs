...
namespace MacPackager
{
    public class Utilities
    {
        ...
        static public string GetPrecompileSourcePListFilename()
        {
            ...
            string SourceName = FileOperations.FindPrefixedFile(Config.BuildDirectory, "Mac-Info.plist");

            if (!File.Exists(SourceName))
            {
                // Try looking for the generic one
                SourceName = FileOperations.FindPrefixedFile(@"..\..\Development\Src\Mac\Resources\", "Mac-Info.plist");
            }
