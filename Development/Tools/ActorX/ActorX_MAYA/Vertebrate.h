...
// version 3: "27 jan 09" is version 3.0  - Added 0 before 1 (so 01 not 1 for january) for future concern
// version 4: "22 april 10" - added per vertex colors for skeletal meshes
...
// If you change this value also change the same define in mayaHelper.h
...
//
// Simple struct for storing a UV coordinate
//
struct FUVCoord
{
	float U,V;
...
//
// PCF BEGIN
// added second uv set - it is alwas filled (zeroed) but writing is skipped if flag OurScene.DoSecondUVSetSkinned is not set (for backward compatibility)
//	unfortunately psk format doesn`t have version parameter
struct VVertex
{
...
// This is the VVertex struct that is exported to the PSK file.  It omits vertex color data since old versions of UE3 would size missmatch since they are not expecting vertex color data.  We will export vertex colors at the bottom of the file.
...
struct VSkin
{
	...
	// True if the skin has vertex colors
...
struct VBlendCurve
{
	...
	VBlendCurve& operator=(VBlendCurve & Other)
	{
		Memzero(this, sizeof(VBlendCurve));
		...
		{
			RawWeightKeys[I] = Other.RawWeightKeys[I];
		}
...
struct VAnimation
{
	...
	void operator=( VAnimation& V ) 
	{
		...
		CurveTrack.Empty();
		for( INT k=0;k<V.CurveTrack.Num(); k++)
		{
...
public:
	...
	// morph curve information
	TArray<VBlendCurve>	RawCurveKeys;
	...
	int SerializeActor(FastFileClass &OutFile)
	{
		...
		// Skin: 3D Points
		Memzero( &ChunkHdr, sizeof( ChunkHdr ) );
		_tcscpy_s(ChunkHdr.ChunkID,("PNTS0000"));
		...
		// Skin: VERTICES (wedges)
		Memzero( &ChunkHdr, sizeof( ChunkHdr ) );
		_tcscpy_s(ChunkHdr.ChunkID,("VTXW0000"));
		ChunkHdr.DataCount = SkinData.Wedges.Num();
		ChunkHdr.DataSize  = sizeof (VVertexExport);
		OutFile.Write( &ChunkHdr, sizeof (ChunkHdr));
		// Copy Wedges to the export struct and put the color data in its own array
		for( int i=0; i<SkinData.Wedges.Num(); i++)
		{
		...
		// Skin: TRIANGLES (faces)
		Memzero( &ChunkHdr, sizeof( ChunkHdr ) );
		_tcscpy_s(ChunkHdr.ChunkID,("FACE0000"));
		...
		// Skin: Materials
		Memzero( &ChunkHdr, sizeof( ChunkHdr ) );
		_tcscpy_s(ChunkHdr.ChunkID,("MATT0000"));
		...
		// Reference Skeleton: Refskeleton.TotalBones times a VBone.
		Memzero( &ChunkHdr, sizeof( ChunkHdr ) );
		_tcscpy_s(ChunkHdr.ChunkID,("REFSKELT"));
	...
	int SerializeAnimation(FastFileClass &OutFile)
	{
		...
		{	
			...
			{
				OutFile.Write( OutAnims[a].CurveTrack[I].RawCurveName, 128 * sizeof(char));
				OutFile.Write( &(OutAnims[a].CurveTrack[I].RawWeightKeys[0]), TrackCurveKeys * sizeof(FLOAT) );
			}
		}
	...
	int LoadAnimation(FastFileClass &InFile)
	{
		...
		UBOOL CurveChunkExpected = ( PSAVersion >= 20090127);
		// Bones
		InFile.Read(&ChunkHdr,sizeof(ChunkHdr));
			...
			if( TotalScaleKeys )
			{
				...
				{	
					...
					if( OutAnims[s].AnimInfo.ScaleInclude ) // ONLY for those sequences with scaling.
					{
						...
						{
							char SeqName[65];
							_tcscpy_s( SeqName, OutAnims[s].AnimInfo.Name );
		...
		if( CurveChunkExpected )
		{
			...
			{	
				...
				{			
					...
					{
						InFile.Read( OutAnims[I].CurveTrack[J].RawCurveName, 128 * sizeof(char));

						OutAnims[I].CurveTrack[J].RawWeightKeys.Add(TrackCurveKeys);
						InFile.Read( &(OutAnims[I].CurveTrack[J].RawWeightKeys[0]), TrackCurveKeys * sizeof(FLOAT) );
					}
				}
				else
				{
	...
	{		
		...
		if( RawNumFrames && RawNumBones ) 
		{
			...
			// add curve into the curve track
			for(INT s=0; s< RawCurveKeys.Num(); s++)
			{
				Animations[ThisIndex].CurveTrack.AddItem( RawCurveKeys[s] );
			}
