	...
	*/		
			...
			switch( LOWORD(wParam) ) 
			{
							...
							if( InSelBufferNum )
							{
								...
								*/							
								...
								// Kludgy elaborate way of detecting different property values for multi-selected animations.
								{
									...
									{
										INT AnimIdx=InSelBuffer[c];									
										_tcscpy_s(instr1, TempActor.Animations[AnimIdx].AnimInfo.Group );									
										if( c>0 ) 
										{
											if( strcmp(instr1,instr2) != 0)
												SameAnimGroups = false;
										}
										_tcscpy_s(instr2,instr1);
									}								
									if( SameAnimGroups )
								...
								//////////			
								{
									...
									{
										INT AnimIdx=InSelBuffer[c];									
										_tcscpy_s(instr1, TempActor.Animations[AnimIdx].AnimInfo.Name );									
										if( c>0 ) 
										{
											if( strcmp(instr1,instr2) != 0)
											{
				...
				*/
					...
					for( INT c=0; c<InSelBufferNum; c++)
					{
						...
						if( (AnimIdx > -1) && (AnimIdx < TempActor.Animations.Num()) )
						{
							...
							if( NameUnique && ConsistentBones )
							{
								...
								TempActor.CopyToOutputAnims(AnimIdx, i);
				...
				case IDC_COPYIN:
				{
					...
					for( INT c=0; c<OutSelBufferNum; c++)
					{
						...
						if( (AnimIdx > -1) && (AnimIdx < TempActor.OutAnims.Num()) )
						{
							...
							TempActor.CopyFromOutputAnims(i, AnimIdx);
							// Completely update listbox contents:
							UpdateInBox = true;
							UpdateOutBox = true;
				...
				{	
					...
					newname[0] = 0;
					GetLoadName( hWnd, newname, _countof(newname), to_path, filterList);
					GetNameFromPath(to_animfile,newname, MAX_PATH );
					_tcscpy_s( DestPath, newname );
				...
				{						
					char newname[MAX_PATH];
					_tcscpy_s(newname,("XX"));
					char defaultextension[]=".psa";
				    GetSaveName( hWnd, newname, _countof(newname), to_path, NULL, defaultextension );
	...
	if( UpdateAnimParams )
	{
		...
		if( UpdateGroup )
		{
			...
			{
				INT AnimIdx=InSelBuffer[c];
				_tcscpy_s( TempActor.Animations[AnimIdx].AnimInfo.Group, in_string );
			}
		}
		...
		if( UpdateName )
		{
			...
			{
				INT AnimIdx=InSelBuffer[c];
				_tcscpy_s( TempActor.Animations[AnimIdx].AnimInfo.Name, in_string );
			}
	...
	switch (msg) 
	{
		case WM_INITDIALOG:
			//CenterWindow(hWnd, GetParent(hWnd)); 
			{
				...
				PrintWindowNum(hWnd,IDC_BONESCURRENT, OurScene.TotalBones);
			}
		break;
