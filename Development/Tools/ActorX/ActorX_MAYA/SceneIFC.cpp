...
int	SceneIFC::LogSkinInfo( VActor *Thing, const char* SkinName ) 
{
	...
	for( INT i=0; i < Thing->SkinData.Materials.Num(); i++)
	{
		...
		_tcscpy_s(MaterialName,CleanString( Thing->SkinData.Materials[i].MaterialName ));
		...
		{
			sprintf_s(BitmapName,"[Internal material]");
			sprintf_s(BitmapPath,"[-]");
		}
...
{		
	...
	char MeshName[200];
	char AnimName[200];
	sprintf_s(MeshName,"%s%s",ScriptName,"Mesh");
	sprintf_s(AnimName,"%s%s",ScriptName,"Anims");
...
void SceneIFC::FixRootMotion(VActor *Thing) //, BOOL Fix, BOOL Lock )
{
	...
	if( ! DoLockRoot )
	{
		...
		FVector EndPos   = Thing->RawAnimKeys[(OurTotalFrames-1)*TotalBones].Position;

		for(int n=0; n<OurTotalFrames; n++)
		{
			float Alpha = ((float)n)/(float(OurTotalFrames-1));
			Thing->RawAnimKeys[n*TotalBones].Position += Alpha*(StartPos - EndPos); // aligns end position onto start.
		}
	}
	...
	if( DoLockRoot )
	{
		...
		{
			//float Alpha = ((float)n)/(float(OurTotalFrames-1));
			Thing->RawAnimKeys[n*TotalBones].Position = StartPos; //Locked
		}
	}
}
