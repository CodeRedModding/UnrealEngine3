...
{
	const size_t LStringLen = strlen(CheckString)+1;
	char *LString = new char[LStringLen];
	_tcscpy_s(LString,LStringLen,CheckString);
	_strlwr_s(LString,LStringLen); //make lower case

	const size_t LFragmentLen = strlen(Fragment)+1;
	char *LFragment = new char[LFragmentLen];
	_tcscpy_s(LFragment,LFragmentLen,Fragment);
	_strlwr_s(LFragment,LFragmentLen); //make lower case
...
INT FindValueTag(const char* CheckString, const char* Fragment, INT DigitNum)
{
	...
	INT N=0;
	SubPos += ( INT )strlen(Fragment); // move beyond fragment.
	INT NumPos = 0;
