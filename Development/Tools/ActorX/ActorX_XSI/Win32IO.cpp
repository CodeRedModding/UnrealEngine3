...
int appGetVarArgs( char* Dest, int Count, char** Fmt )
{
    /*va_list ArgPtr;
	va_start( ArgPtr, test );
...
void PopupBox(char* PrintboxString, ... )
{
	char TempStr[4096];
	va_list ArgPtr;
	va_start( ArgPtr, PrintboxString );
	INT Result = _vsnprintf( TempStr, 4096, PrintboxString, ArgPtr );
	va_end( ArgPtr );
