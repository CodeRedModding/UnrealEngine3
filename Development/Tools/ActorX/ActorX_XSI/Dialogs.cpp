				...
				*/
				...
				{				
					...
					if( newname[0] ) // Nonzero string = OK name.
					{
						...
						{
							// Save animation using newname (includes path).
							FastFileClass OutFile;
							if ( OutFile.CreateNewFile(newname) != 0) // Error!
							{
							...
							else
							{
								...
								OutFile.CloseFlush();
								if( OutFile.GetError()) ErrorBox("Animation File Save Error.");
							}
					...
					//PopupBox("Selected name: %s length: %i", newname, strlen(newname) );
				}
				break;
