	...
	private function processAlignment():Void {
		...
		for (var i:Number = 0; i < _elements.length; i++) {
			...
			switch (element.hconstrain) {
				case StagePanel.LEFT: { 	clip._x = metrics.left + hoffset; 	break; }
				case StagePanel.RIGHT: {	clip._x = metrics.left - hoffset; 	break; }
				case StagePanel.ALL: {		
			...
			switch (element.vconstrain) {
				case StagePanel.TOP: {		clip._y = metrics.top + voffset;	break; }
				case StagePanel.BOTTOM: {	clip._y = metrics.top - voffset;	break;	}
				case StagePanel.ALL: {
