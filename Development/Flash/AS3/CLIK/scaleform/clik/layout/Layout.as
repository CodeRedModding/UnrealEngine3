            ...
            for (var i:uint = 0; i < _managedSprites.length; i++) {
                ...
                if (alignH != LayoutMode.ALIGN_NONE) {
                    if (alignH == LayoutMode.ALIGN_LEFT) {
                        ...
                        // from the Layout on the Stage.
                        if (relHObject == null) {
                            spr.x = ((rect.right + rect.left) / 2) - (spr.width / 2) + offsetH;
                        }
                        // The object will be aligned to relHObject. If the offset is -1, use it's offset
                        // from the Sprite on the Stage.
                        else {
							spr.x = (relHObject.x + (relHObject.width / 2)) - (spr.width / 2) + offsetH;
                        }
                    }
					else if (alignH == LayoutMode.ALIGN_LEFT_BORDERS) {
						// The object will be aligned to the left of our rect. If the offset is -1, use it's offset
                        // from the Layout on the Stage. This is not intended to be used in this manner, but this is a fall back case
						if (relHObject == null) {
							spr.x = rect.x + offsetH;
						}
						else {
						// The object will be aligned to relHObject so that both left borders are aligned. If the offset is -1, use it's offset
						// from the Sprite on the Stage.
							spr.x = relHObject.x + offsetH;
						}
					}
					else if (alignH == LayoutMode.ALIGN_RIGHT_BORDERS) {
						// The object will be aligned to the right of our rect. If the offset is -1, use it's offset
                        // from the Layout on the Stage. This is not intended to be used in this manner, but this is a fall back case
						if (relHObject == null) {
							spr.x = rect.x + offsetH;
						}
						else {
						// The object will be aligned to relHObject so that both right borders are aligned. If the offset is -1, use it's offset
						// from the Sprite on the Stage.
							spr.x = relHObject.x + relHObject.width - spr.width + offsetH;
						}
					}
                }
                ...
                if (alignV != LayoutMode.ALIGN_NONE) {
                    if (alignV == LayoutMode.ALIGN_TOP) {
						...
						// The object will be aligned to our rect. If the offset is -1, use it's offset from the Layout on the Stage.
                        if (relVObject == null) {
                            spr.y = ((rect.bottom + rect.top) / 2) - (spr.height / 2) + offsetV;
                        }
                        // The object will be algined to relVObject. If the offset is -1, use it's offset from the Sprite on the Stage.
                        else {
							spr.y = (relVObject.y + ( relVObject.height / 2)) - (spr.height / 2) + offsetV;
                        }
                    }
					else if (alignV == LayoutMode.ALIGN_TOP_BORDERS) {
						// The object will be aligned to the top of our rect. If the offset is -1, use it's offset
                        // from the Layout on the Stage. This is not intended to be used in this manner, but this is a fall back case
						if (relVObject == null) {
							spr.y = rect.y + offsetV;
						}
						else {
						// The object will be aligned to relHObject so that both top borders are aligned. If the offset is -1, use it's offset
						// from the Sprite on the Stage.
							spr.y = relVObject.y + offsetV;
						}
					}
					else if (alignV == LayoutMode.ALIGN_BOTTOM_BORDERS) {
						// The object will be aligned to the bottom of our rect. If the offset is -1, use it's offset
                        // from the Layout on the Stage. This is not intended to be used in this manner, but this is a fall back case
						if (relVObject == null) {
							spr.y = rect.y + offsetV;
						}
						...
						// The object will be aligned to relHObject so that both right borders are aligned. If the offset is -1, use it's offset
						// from the Sprite on the Stage.
							spr.y = relVObject.y + relVObject.height - spr.height + offsetV;
						}
					}
                }
            }
